...
- 当修改 `.github/workflows/` 下的文件时，必须检查并分析是否需要同步更新 `.forgejo/workflows/` 中的对应文件。两个目录中的同名 workflow 应保持功能一致性（Forgejo 版本需移除 GitHub 特定代码，如 `actions/upload-artifact`、`actions/github-script`、ghcr.io/aliyun/docker hub 登录等）。
