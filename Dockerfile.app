...
# copy local tgz packages referenced by packages/service/package.json + pnpm-lock.yaml via /app/*.tgz
...
# 将 lockfile 里写死的 npmmirror tarball 重写到 sangfor nexus，避免 ECONNREFUSED
RUN sed -i 's#https://registry\.npmmirror\.com/#http://mirrors.sangfor.org/nexus/repository/npm/#g' pnpm-lock.yaml

# if proxy exists, set proxy
RUN if [ -z "$proxy" ]; then \
        pnpm i; \
    else \
        pnpm i --registry=http://mirrors.sangfor.org/nexus/repository/npm; \
    fi
...
# copy data dir (preserve config.local.json, metric.json alongside the three required files)
