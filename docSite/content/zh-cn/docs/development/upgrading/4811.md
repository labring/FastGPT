...
10. 优化 - 工作流嵌套层级限制 20 层，避免因编排不合理导致的无限死循环。
11. 优化 - 工作流 handler 性能优化。
12. 优化 - 工作流快捷键，避免调试测试时也会触发。
13. 优化 - 流输出，切换 tab 时仍可以继续输出。
14. 优化 - 完善外部文件知识库相关 API
15. 修复 - 知识库选择权限问题。
16. 修复 - 空 chatId 发起对话，首轮携带用户选择时会异常。
17. 修复 - createDataset 接口，intro 为赋值。
