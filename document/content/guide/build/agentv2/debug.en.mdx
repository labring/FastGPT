...
description: Detailed guide to generating Prompts via the AI Helper Bot and testing and debugging the Agent using the Chat Preview window and run details.
---
...
The "Chat Preview" tab provides a real-time conversational testing environment, allowing you to interact with the Agent as a real user and verify the app's effectiveness before publishing.
...
- **Bubble Action Bar and Run Details**: Below each generated AI chat bubble, a row of auxiliary debugging tools is provided:
  ...
  - **Annotation**: Lets developers annotate the question and expected answer, saving them to a designated Dataset to improve future responses.
  - **Retry**: Triggers the AI to regenerate the response for the last user input.
  - **Run Details**: Click to expand the tree-structured decision-making chain. This logs the complete LLM reasoning process, internal plan updates, and tool call logs. It also shows the unique Request ID, model name, response duration, and precise points consumed for performance auditing and cost control.
