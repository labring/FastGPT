...
为了方便地接入**外部成员系统**，AI Platform 提供一套接入外部系统的**标准接口**，以及一个 FastGPT-SSO-Service 镜像作为**适配器**。

通过这套标准接口，你可以可以实现：

1. SSO 登录。从外部系统回调后，在 AI Platform 中创建一个用户。
...
- 成员所属部门会同步过滤为应用可见部门；只属于不可见部门的成员不会同步到 AI Platform。
