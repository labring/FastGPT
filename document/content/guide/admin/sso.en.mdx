...
<Alert context="info">
  This address is operator-configured and is a trusted internal endpoint rather than user-controlled
  input. SSO authorization, the OAuth callback, and member sync all connect to it directly and are
  unaffected by `CHECK_INTERNAL_IP`; enabling `CHECK_INTERNAL_IP=true` does not break SSO login or
  member sync.
</Alert>

#### 2. In the FastGPT frontend, go to `Admin` -> `System Configuration` -> `User Configuration` to configure button text, icons, etc.
...
Enable member synchronization in the FastGPT frontend under `Admin` -> `System Configuration` -> `User Configuration`. SYNC_MEMBER_CRON uses UTC; leaving it empty disables scheduled synchronization.
...
1. Self-development: Build according to the standard interfaces provided by FastGPT, then enter the deployed service address in the FastGPT frontend under `Admin` -> `System Configuration` -> `User Configuration`.
