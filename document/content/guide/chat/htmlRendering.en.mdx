...
This feature extends AI Platform's Markdown rendering to support embedded HTML content. Since rendering uses an iframe, the content height cannot be determined automatically, so AI Platform sets a fixed height for the iframe. JavaScript execution within the HTML is not supported.
