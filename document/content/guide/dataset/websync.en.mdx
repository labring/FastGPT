...
Web Site Sync uses crawler technology to automatically discover all pages under the `same domain` from an entry URL, supporting up to `200` sub-pages. For compliance and security reasons, FastGPT only supports crawling `static sites`, primarily intended for quickly building datasets from documentation sites.
...
In the earlier demo, we used multiple selectors for the AI Platform documentation site, separated by commas.
