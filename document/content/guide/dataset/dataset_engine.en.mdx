...
AI Platform uses an embedding-based RAG approach to build Datasets. To use AI Platform effectively, you need a basic understanding of how embedding vectors work and their characteristics.
...
In AI Platform, a Dataset contains Collections, and each Collection contains data entries. A Collection can be thought of as a file. Dataset search runs across the entire Dataset; Collections organize and manage data but do not currently limit the search scope.
...
In a single vector, content length and semantic richness are often at odds. AI Platform uses multi-vector mapping to map a single data entry to multiple vectors, preserving both data completeness and semantic richness.
...
A Dataset search is not simply "user question -> vector database -> result." Depending on the input and search parameters, AI Platform combines text, images, semantic recall, full-text recall, query rewriting, and reranking, then fuses multiple result paths into the final cited content.
...
In Dataset search, images can participate in retrieval in addition to text questions. AI Platform handles images differently depending on the configured model capabilities.
...
AI Platform fuses results from different recall paths instead of using only one path. Common paths include text vector recall, full-text recall, image description recall, image vector recall, and reranked results.
