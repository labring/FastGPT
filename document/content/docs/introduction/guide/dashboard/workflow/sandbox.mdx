...
内置函数通过 `SystemHelper` 对象调用。JS 中使用 `SystemHelper.xxx()`，Python 中使用 `system_helper.xxx()`。

> 直接调用 `countToken()` / `count_token()` 等方式已弃用，仅保留向后兼容。新代码请统一使用 `SystemHelper` / `system_helper`。
...
# Python
def main():
    system_helper.delay(1000)  # 延迟 1 秒
    return {"result": "done"}
```
...
# Python
def main():
    return {"tokens": system_helper.count_token(variables["input"])}
```
...
def main():
    return {
        "image": system_helper.str_to_base64(variables["svg"], "data:image/svg+xml;base64,")
    }
```
...
# Python
def main():
    result = system_helper.create_hmac("sha256", variables["secret"])
...
# Python
def main():
    res = system_helper.http_request(variables["url"], method="GET", headers={}, timeout=10)
...
|------|-----------|--------|
| 统计 token | `SystemHelper.countToken(text)` | `system_helper.count_token(text)` |
| 字符串转 base64 | `SystemHelper.strToBase64(text, prefix?)` | `system_helper.str_to_base64(text, prefix?)` |
| HMAC 签名 | `SystemHelper.createHmac(algo, secret)` | `system_helper.create_hmac(algo, secret)` |
| 延迟 | `await SystemHelper.delay(ms)` | `system_helper.delay(ms)` |
| HTTP 请求 | `await SystemHelper.httpRequest(url, opts?)` | `system_helper.http_request(url, method?, headers?, body?, timeout?)` |
