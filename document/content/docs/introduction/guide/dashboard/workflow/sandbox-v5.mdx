...
> 本文档适用于 FastGPT **4.14.8 及以上版本**。4.14.7 及以下版本请参考 [代码运行（旧版）](/docs/introduction/guide/dashboard/workflow/sandbox)。
...
内置函数统一通过 `SystemHelper` 对象调用，JS 和 Python 使用相同的 API。

> 直接调用 `countToken()` / `count_token()` 等方式已弃用，仅保留向后兼容。新代码请统一使用 `SystemHelper`。
...
// JavaScript
async function main({data1, data2}){
    await SystemHelper.delay(1000) // 延迟 1 秒
    return { result: "done" }
}
```
...
# Python
def main():
    SystemHelper.delay(1000)  # 延迟 1 秒
    return {"result": "done"}
```
...
// JavaScript
function main({input}){
    return { tokens: SystemHelper.countToken(input) }
}
```
...
# Python
def main():
    return {"tokens": SystemHelper.countToken(variables["input"])}
```
...
function main({svg}){
    return {
        // 参数 1: 要转换的字符串
        // 参数 2: base64 前缀（可选）
        image: SystemHelper.strToBase64(svg, 'data:image/svg+xml;base64,')
    }
}
```
...
def main():
    return {
        "image": SystemHelper.strToBase64(variables["svg"], "data:image/svg+xml;base64,")
    }
```
...
function main({secret}){
    const {sign, timestamp} = SystemHelper.createHmac('sha256', secret)
...
# Python
def main():
    result = SystemHelper.createHmac("sha256", variables["secret"])
...
# Python
def main():
    res = SystemHelper.httpRequest(variables["url"], method="GET", headers={}, timeout=10)
...
| 功能 | JavaScript (SystemHelper) | Python (system_helper) |
|------|-----------|--------|
| 统计 token | `SystemHelper.countToken(text)` | `SystemHelper.countToken(text)` |
| 字符串转 base64 | `SystemHelper.strToBase64(text, prefix?)` | `SystemHelper.strToBase64(text, prefix?)` |
| HMAC 签名 | `SystemHelper.createHmac(algo, secret)` | `SystemHelper.createHmac(algo, secret)` |
| 延迟 | `await SystemHelper.delay(ms)` | `SystemHelper.delay(ms)` |
| HTTP 请求 | `await SystemHelper.httpRequest(url, opts?)` | `SystemHelper.httpRequest(url, method?, headers?, body?, timeout?)` |
