...
这里提供了 FastGPT 目前支持的对象存储厂商，包括自部署的 MinIO、AWS S3、阿里云 OSS、腾讯云 COS 和百度云 BOS 的环境变量配置说明
...
- `STORAGE_VENDOR` 该变量为一个枚举值，可选值为：`minio`、`aws-s3`、`oss`、`cos` 和 `bos`。
...
> - 除了通用环境变量外，还需要额外配置 `STORAGE_BOS_ENDPOINT`。
> - 使用 BOS 时建议保持 `STORAGE_S3_FORCE_PATH_STYLE=true`。

- `STORAGE_BOS_ENDPOINT` 百度云 BOS 的 S3 兼容 Endpoint，例如北京区域可填写 `https://s3.bj.bcebos.com`
