...
这里提供了 FastGPT 目前支持的对象存储厂商，包括自部署的 MinIO、AWS S3、百度云 BOS、阿里云 OSS 和腾讯云 COS 的环境变量配置说明
...
- `STORAGE_VENDOR` 该变量为一个枚举值，可选值为：`minio`、`aws-s3`、`bos`、`oss` 和 `cos`。
...
### 自部署的 MinIO、AWS S3 和兼容 S3 协议的对象存储
...
> 如果使用百度云 BOS，请将 `STORAGE_VENDOR` 设置为 `bos`，并优先填写 `STORAGE_BOS_ENDPOINT`。

- `STORAGE_S3_ENDPOINT` 内网连接地址，可以是容器 id 连接，比如 `http://fastgpt-minio:9000`
- `STORAGE_BOS_ENDPOINT` 【BOS 可选】百度云 BOS 的 S3 兼容 Endpoint，例如北京区域可填写 `https://s3.bj.bcebos.com`
...
- `STORAGE_S3_FORCE_PATH_STYLE` 【可选】虚拟主机风格路由或路径路由风格，其中如果厂商填写了 `minio` 的话，该值被固定为 `true`；使用 BOS 时建议保持 `true`
...
> - BOS 通过 S3 兼容接口接入 FastGPT。
> - 请使用百度云官方已提供 S3 兼容 Endpoint 的区域，例如 `bj`、`gz`、`su`。
> - 使用 BOS 时建议保持 `STORAGE_S3_FORCE_PATH_STYLE=true`。
