...
This guide covers environment variable configuration for object storage providers supported by FastGPT, including self-hosted MinIO, AWS S3, Baidu BOS, Alibaba Cloud OSS, and Tencent Cloud COS.
...
- `STORAGE_VENDOR` Enum value. Options: `minio`, `aws-s3`, `bos`, `oss`, `cos`.
...
### Self-Hosted MinIO, AWS S3, and Other S3-Compatible Storage
...
> If you are using Baidu BOS, set `STORAGE_VENDOR` to `bos` and prefer `STORAGE_BOS_ENDPOINT`.

- `STORAGE_S3_ENDPOINT` Internal connection address. Can be a container ID, e.g., `http://fastgpt-minio:9000`
- `STORAGE_BOS_ENDPOINT` [BOS Optional] Baidu BOS S3-compatible endpoint. For example, the Beijing region can use `https://s3.bj.bcebos.com`
...
- `STORAGE_S3_FORCE_PATH_STYLE` [Optional] Virtual-hosted-style or path-style routing. If vendor is `minio`, this is fixed to `true`. For BOS, keeping this as `true` is recommended.
...
> - BOS is integrated through its S3-compatible API.
> - Use a region that has an official BOS S3-compatible endpoint, such as `bj`, `gz`, or `su`.
> - When using BOS, keep `STORAGE_S3_FORCE_PATH_STYLE=true`.
...
STORAGE_PUBLIC_BUCKET=fastgpt-public
STORAGE_PRIVATE_BUCKET=fastgpt-private
STORAGE_BOS_ENDPOINT=https://s3.bj.bcebos.com
STORAGE_S3_FORCE_PATH_STYLE=true
STORAGE_S3_MAX_RETRIES=3
```
