...
This guide covers environment variable configuration for object storage providers supported by FastGPT, including self-hosted MinIO, AWS S3, Alibaba Cloud OSS, Tencent Cloud COS, and Baidu BOS.
...
- `STORAGE_VENDOR` Enum value. Options: `minio`, `aws-s3`, `oss`, `cos`, `bos`.
...
> - In addition to the common environment variables, configure `STORAGE_BOS_ENDPOINT`.
> - When using BOS, keep `STORAGE_S3_FORCE_PATH_STYLE=true`.

- `STORAGE_BOS_ENDPOINT` Baidu BOS S3-compatible endpoint. For example, the Beijing region can use `https://s3.bj.bcebos.com`
...
STORAGE_PUBLIC_BUCKET=fastgpt-public
STORAGE_PRIVATE_BUCKET=fastgpt-private
STORAGE_EXTERNAL_ENDPOINT=https://s3.bj.bcebos.com
