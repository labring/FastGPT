---
title: WeCom Bot Integration Tutorial
description: FastGPT WeCom Bot Integration Tutorial
---

- Starting from version 4.12.4, FastGPT commercial version supports direct WeCom bot integration without additional APIs.
- Starting from version 4.14.4, FastGPT cloud service version supports WeCom intelligent bot integration through custom domain configuration.

## 1. (Required for Cloud Service Version) Configure Custom Domain

WeCom requires intelligent bot message push addresses to use the enterprise's primary domain, so cloud service version users must configure a custom domain before using WeCom bots.

- [Configure Custom Domain](/docs/introduction/guide/team_permissions/customDomain)

If you're a commercial version user, continue using your enterprise domain.
...
### 2.2 Find Intelligent Bot Entry
...
### 2.3 Select "API Mode Creation" for Intelligent Bot
...
Configure the publishing channel information. You need to enter the Token and AESKey recorded in step 4 (Token and Encoding-AESKey)
...
3. Check FastGPT chat logs to see if there's a corresponding question record.
4. If there's no record, the application may have encountered an error. Try the simplest bot first.
