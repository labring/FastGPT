...
您需要在计算机上安装和配置以下依赖项才能构建 <FastGPTLink campaign="docs_self_host_dev" content="prerequisites_product_link">AI Platform</FastGPTLink>：
...
若您本地已经通过 docker 启动了 AI Platform，则需要先关闭，否则会有端口冲突。
...
3. 向 AI Platform 的 main 分支提交一个 PR，提交请求后，AI Platform 团队/社区的其他人将与您一起审查它。
...
AI Platform 采用 pnpm workspace 方式构建 monorepo 项目，主要分为两个部分：

- projects/app - AI Platform 主项目
- packages/ - 子模块
...
AI Platform 在代码模块划分时，按 DDD 的思想进行划分，主要分为以下几个领域：
...
├── projects
│   └── app                      // AI Platform 主项目
├── python                       // 存放一些模型代码，和 AI Platform 本身无关
└── scripts                      // 一些自动化脚本
