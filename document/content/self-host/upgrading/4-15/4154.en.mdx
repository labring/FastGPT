...
- Removes only historical FastGPT indexes explicitly marked as deprecated by the corresponding schema and whose name, key, and relevant options match exactly.
...
Setting `SYNC_INDEX=false` skips missing-index creation and any deprecated-index cleanup that may be introduced in future releases. You must then ensure that the database indexes meet the requirements of the current FastGPT version.
