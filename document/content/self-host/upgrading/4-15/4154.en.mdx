...
AI Platform, including the scheme, host, and optional port. Use the public client-facing origin in
...
AI Platform now performs safe index synchronization automatically at startup:

- Creates indexes that are missing from the current AI Platform schemas.
- Removes only built-in historical AI Platform indexes explicitly marked as deprecated by the corresponding schema and whose name, key, and relevant options match exactly.
