...
- `presigned`：直接返回 S3 预签名链接，与此前配置了 `STORAGE_EXTERNAL_ENDPOINT` 时的行为一致。

如果现有部署依赖 S3/CDN 直连下载，请显式配置：
...
使用 `short-redirect` 时，还需要配置
`STORAGE_EXTERNAL_ENDPOINT` 或 `STORAGE_S3_CDN_ENDPOINT`。
