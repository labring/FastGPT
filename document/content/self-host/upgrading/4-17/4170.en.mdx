...
V4.17.0 adds a `createTime` field to Apps. Workbench sorting by recently created or oldest created depends on this field. Newly created, copied, or imported Apps write `createTime` automatically. Historical records require a one-time upgrade endpoint, which backfills the timestamp stored in each document `_id` (ObjectId). Existing `createTime` values are not overwritten. Records whose `_id` is not a valid ObjectId are skipped and counted in `skippedInvalidId`.

Start with a dry run, which does not write any data:

```bash
curl -X POST 'https://your-domain/api/admin/4163/initAppCreateTime' \
  -H 'Content-Type: application/json' \
  -H 'rootkey: YOUR_ROOT_KEY' \
  -d '{"dryRun":true}'
```

After confirming `scannedRecords`, run the backfill:
...
In the response, `scannedRecords` is the number of records missing `createTime`, `updatedRecords` is the number actually written, and `skippedInvalidId` is the number that could not derive a time from `_id`. The endpoint is safe to run repeatedly. After applying the migration, run another dry run: `scannedRecords` should be `0` in the normal case. Remaining `skippedInvalidId` records can be ignored or inspected separately.
