...
18. Improved memory and concurrency control for file parsing. Queued tasks now keep only a lightweight file source instead of the full Buffer. Trusted S3 files reserve resources from their known size, while external URLs update their reservation as bytes stream in and enforce both file-size and per-task memory hard limits. This reduces OOM risk during concurrent parsing.
19. Unified external-file download and parsing across Datasets, Chat, and Workflows. Dataset template and backup files and App evaluation CSVs are uploaded to S3 before parsing, so queued work does not retain multipart temporary files.
