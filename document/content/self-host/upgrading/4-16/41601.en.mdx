...
### 3. Image environment variable changes

Environments that enable OpenSandbox must update the following environment variables:

- `AGENT_SANDBOX_OPENSANDBOX_IMAGE_REPO` and `AGENT_SANDBOX_OPENSANDBOX_IMAGE_TAG` are no longer supported. Remove them and set the full image in `AGENT_SANDBOX_OPENSANDBOX_IMAGE`, for example `registry.cn-hangzhou.aliyuncs.com/fastgpt/fastgpt-agent-sandbox:v0.2.0`; this variable is required when `opensandbox` is enabled, and startup fails if it is missing.
- Volume Manager no longer reads `VM_VOLUME_NAME_PREFIX`. Configure `AGENT_SANDBOX_OPENSANDBOX_VOLUME_NAME_PREFIX` on both `fastgpt-app` and `fastgpt-pro`, and remove `VM_VOLUME_NAME_PREFIX` from the volume-manager service. Keep the previous value when upgrading so existing persistent volumes can still be cleaned up by their original names.
...
### 5. Migrate manual HTTP tool data
