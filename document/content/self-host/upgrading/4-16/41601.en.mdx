...
6. Knowledge Base data supports custom `metadata`, which can be imported as JSON through the API, CSV templates, or Excel templates. Search results and backup exports preserve this field. CSV and Excel templates use `q`, `a`, `index`, and `metadata` headers; `q`, `a`, and `metadata` each use one column, while `index` may use multiple columns in any order. Excel files must contain a single worksheet with no merged cells. Template and backup imports also accept the legacy `q`, `a`, and `indexes` headers.
