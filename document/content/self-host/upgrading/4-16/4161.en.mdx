...
Environments that already completed `initHttpToolSchema` on 4.16.0 do not need to run it again. Version 4.16.1 checks the HTTP tool applications and historical versions covered by the 4.16.0 migration; it refuses to run when legacy array types remain there, so they are not converted directly to strings. HTTP nodes inside workflow applications are outside the 4.16.0 migration scope and are handled by the 4.16.1 schema storage migration.

V4.16.1 stores workflow tool JSON Schemas as strings. Run a dry run first:
