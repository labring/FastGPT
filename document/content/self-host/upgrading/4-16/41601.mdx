...
本版本 `fastgpt-app` 和 `fastgpt-pro` 还新增以下可选配置：
...
### 2. 更新 Agent-sandbox-proxy 环境变量（可选）
...
启用 OpenSandbox 的环境需要同步调整以下环境变量：

- `AGENT_SANDBOX_OPENSANDBOX_IMAGE_REPO` 和 `AGENT_SANDBOX_OPENSANDBOX_IMAGE_TAG` 已移除，不再兼容。请删除这两个变量，改为配置完整的 `AGENT_SANDBOX_OPENSANDBOX_IMAGE`，例如 `registry.cn-hangzhou.aliyuncs.com/fastgpt/fastgpt-agent-sandbox:v0.2.0`；启用 `opensandbox` 时该项为必填，未配置会阻止服务启动。
- Volume Manager 不再读取 `VM_VOLUME_NAME_PREFIX`。请在 `fastgpt-app` 和 `fastgpt-pro` 中配置 `AGENT_SANDBOX_OPENSANDBOX_VOLUME_NAME_PREFIX`，并从 volume-manager 服务中移除 `VM_VOLUME_NAME_PREFIX`；升级时需沿用原值，否则历史持久卷可能无法按原名称清理。
