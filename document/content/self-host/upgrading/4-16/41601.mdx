...
6. 知识库数据支持自定义 `metadata`，可通过 API、CSV 或 Excel 模板导入 JSON 元数据；检索结果和备份导出会保留该字段。CSV 和 Excel 模板使用 `q`、`a`、`index`、`metadata` 表头，`q`、`a`、`metadata` 各一列，`index` 可多列且顺序任意；Excel 仅支持单个工作表且不能包含合并单元格。模板导入和备份导入同时兼容旧版 `q`、`a`、`indexes` 表头。
7. 大文件分块上传。
