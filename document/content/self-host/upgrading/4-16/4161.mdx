---
title: 'V4.16.1'
description: 'FastGPT V4.16.1 更新说明'
upgradeTags:
  - 进行中
---

## 🚀 新增内容

1. 第三方发布渠道，支持多模态文件、音频转录、引用（如有）
2. 支持团队安装系统插件（进程模式，存在一定安全风险，未来版本支持 serverless 隔离运行）。
3. 沙盒支持自定义 apt 源。

## ⚙️ 优化

1. S3 key 命名，取消原文件名作为 key，防止超长。

## 🐛 修复

1. 工作流全局变量配置，字段名太长溢出。

## 🛠️ 代码优化

1. 补全所有 devapi 接口文档。

## 🔧 升级迁移

### 迁移工具 JSON Schema 存储格式

请严格按以下版本顺序执行迁移：

1. 在 4.16.0 版本先完成 `initHttpToolSchema`，将手动 HTTP 工具的历史数组参数转换为标准 JSON Schema。
2. 升级到 4.16.1 后，再执行本节的 `initToolJsonSchemaStorage`。

已经在 4.16.0 完成 `initHttpToolSchema` 的环境无需重复执行。4.16.1 会检查 4.16.0 负责的 HTTP 工具应用及其历史版本；如果这些数据仍包含旧数组类型，接口会拒绝执行，避免旧数据被直接转换为字符串。工作流应用内的 HTTP 节点不属于 4.16.0 的迁移范围，仍由 4.16.1 的 Schema 存储迁移处理。

4.16.1 将工作流工具的 JSON Schema 统一为字符串存储。请先执行 dry-run：

```bash
curl -X POST 'https://你的域名/api/admin/4161/initToolJsonSchemaStorage' \
  -H 'Content-Type: application/json' \
  -H 'rootkey: 你的ROOT_KEY' \
  -d '{"dryRun":true}'
```

确认 `total.changedDocumentCount` 后执行正式迁移：

```bash
curl -X POST 'https://你的域名/api/admin/4161/initToolJsonSchemaStorage' \
  -H 'Content-Type: application/json' \
  -H 'rootkey: 你的ROOT_KEY' \
  -d '{"dryRun":false}'
```
