...
10. 修复旧版 App 模板中的历史工作流字段无法按当前结构读取的问题。
11. 修复文件图标使用模糊后缀匹配，导致 Markdown 文件名包含 DOCX 等文本时错误显示为 DOC 图标的问题；补齐新增文档格式的图标映射，并为 EPUB 增加独立图标。
12. 修复历史 App 变量的 `valueType` 为空或 `null` 时，聊天配置无法通过校验的问题。读取和保存聊天配置时会根据变量输入类型补齐 `valueType`，无法映射或值非法时回退为 `any`，并在保存前通过 Zod 校验；已有数据无需执行额外清洗脚本。
