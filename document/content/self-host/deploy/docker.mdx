...
非交互模式还必须通过 `FASTGPT_FE_DOMAIN` 指定用户访问 FastGPT 的完整地址，例如 `https://fastgpt.example.com`。启用 Agent/Skill 沙盒的版本还需要通过 `FASTGPT_SANDBOX_PROXY_URL` 指定沙盒 WebSocket 地址，例如 `wss://sandbox-proxy.example.com`；4.16 还需要通过 `FASTGPT_SANDBOX_PREVIEW_PROXY_URL` 指定 HTTP 预览地址。v4.14 不包含该服务。交互模式下脚本会按版本询问这些地址；4.15 只询问 WebSocket 地址。

访问地址和宿主机端口是独立配置。需要直连时，可通过 `FASTGPT_PORT` 设置 FastGPT 宿主机端口（默认 `3000`），通过 `FASTGPT_SANDBOX_PROXY_PORT` 设置 Sandbox Proxy 宿主机端口（默认 `3006`）；容器端口仍分别为 `3000` 和 `1006`。如果使用反向代理，`FASTGPT_FE_DOMAIN`、`FASTGPT_SANDBOX_PROXY_URL` 及预览地址中的公网端口（例如 `443`）不会被当作宿主机端口，脚本会保留 Compose 中的默认映射。
...
1. 默认 `3000` 端口（FastGPT 主服务；可通过 `FASTGPT_PORT` 修改宿主机端口）
2. 9000 端口（S3 服务）
3. 默认 `3006` 端口（FastGPT Agent Sandbox Proxy 服务；可通过 `FASTGPT_SANDBOX_PROXY_PORT` 修改宿主机端口）
4. 3003 端口（AI Platform SSE MCP server 服务，不需要的话可以不开通。）
...
从 V4.14.0 版本开始，fastgpt-plugin 镜像仅提供运行环境，不再预装系统插件，所有 AI Platform 系统需手动安装系统插件。

- 通过插件市场安装，默认会向公开的 AI Platform Marketplace 获取数据进行安装。
...
| FastGPT-plugin 版本 | AI Platform 主服务       |
| ------------------- | -------------------- |
| 1.x                 | 4.15.x               |
...
从 V4.17.0 起，**必须部署或接入 AI Proxy**，并在 AI Platform 容器中配置以下环境变量：

- `AIPROXY_API_ENDPOINT`：FastGPT 可访问的 AI Proxy 服务根地址，例如 `http://aiproxy:3000`，不要附加 `/v1`；FastGPT 会按接口补充路径。
...
可以通过修改 `docker-compose.yml` 中 FastGPT 容器的环境变量更新 AI Proxy 连接配置；商业版 Admin 服务若复用 App/Service 配置，也需要设置这两个变量。
...
PG 数据库没有连接上/初始化失败，可以查看日志。AI Platform 会在每次连接上 PG 时进行表初始化，如果报错会有对应日志。
