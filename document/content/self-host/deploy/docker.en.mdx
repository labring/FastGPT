...
1. Port 3000 by default (FastGPT main service; change the host port with `FASTGPT_PORT`)
2. Port 9000 (S3 service)
3. Port 3006 by default (FastGPT Agent Sandbox Proxy service; change the host port with `FASTGPT_SANDBOX_PROXY_PORT`)
4. Port 3003 (AI Platform SSE MCP server service; you can leave this port closed if you don't need it.)
...
Access AI Platform via the port/domain opened in step 3.
...
Starting from V4.14.0, the fastgpt-plugin image only provides the runtime environment without pre-installed system plugins. All AI Platform systems must manually install system plugins.

- Install via the plugin marketplace — by default it fetches from the public AI Platform Marketplace.
...
| FastGPT-plugin Version | AI Platform Main Service |
| ---------------------- | -------------------- |
| 1.x                    | 4.15.x               |
...
Starting with V4.17.0, **you must deploy or connect to AI Proxy** and set these environment variables in the AI Platform container:

- `AIPROXY_API_ENDPOINT`: The AI Proxy service root URL reachable from FastGPT, such as `http://aiproxy:3000`. Do not append `/v1`; FastGPT adds the path required by each API.
...
To update the AI Proxy connection, edit the FastGPT container's environment variables in `docker-compose.yml`. Commercial Admin services that share App/Service configuration also need both variables.
...
PG database not connected or initialization failed — check logs. AI Platform initializes tables on each PG connection. Errors will appear in the logs.
