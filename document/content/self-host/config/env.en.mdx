...
| `AGENT_SANDBOX_SEALOS_IMAGE`                 | Empty                    | Image used by Sealos Devbox.                                                                                                                                                                       |
...
| `AGENT_SANDBOX_VOLUME_MANAGER_URL`           | `http://localhost:3005`  | Volume Manager service URL.                                                                                                                                                                        |
...
| `AGENT_SANDBOX_ARCHIVE_MAX_SIZE`             | `50`                     | Maximum Agent sandbox cold archive package size, in MB. Used for archive upload, download, and packaging checks.                                                                                   |
...
| `CHAT_TITLE_MODEL`                     | Empty     | Chat title generation model. If unset or not matched to an enabled model, the system falls back to the default LLM.                                                           |
...
These variables are loaded and validated by `projects/volume-manager/src/env.ts`. When persistent OpenSandbox volumes are enabled on the FastGPT side, `AGENT_SANDBOX_VOLUME_MANAGER_TOKEN` must match `VM_AUTH_TOKEN`.
...
| `VM_K8S_PVC_STORAGE_SIZE`  | `1Gi`                  | Kubernetes PVC size. Required only in `kubernetes` mode.            |
| `VM_VOLUME_NAME_PREFIX`    | `fastgpt-session`      | Volume/PVC name prefix. The final name includes the sessionId hash. |
