...
| Variable | Default | Description |
| --- | --- | --- |
| `PLUGIN_BASE_URL` | Empty | FastGPT Plugin service URL. Deployment templates usually set this to the internal Plugin service URL. |
| `PLUGIN_TOKEN` | Empty | Authentication token for calling the Plugin service. It must match the Plugin service configuration. |
| `CODE_SANDBOX_URL` | Empty | Code Sandbox service URL. Deployment templates usually set this to the internal Code Sandbox service URL. |
| `CODE_SANDBOX_TOKEN` | Empty | Token used by App when calling Code Sandbox. It must match the sandbox service `SANDBOX_TOKEN`. |
