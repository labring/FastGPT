...
| `CHECK_INTERNAL_IP`                 | `false` | Whether internal IP checks are enabled to reduce SSRF risk. Applies only to outbound URLs controlled by users or workflows; operator-configured service addresses (SSO/external user system, model addresses, Feishu/DingTalk/Yuque base URLs, custom PDF parse, custom censor, fast login, chat log, SMS proxy, invoice webhook, enterprise auth, etc.) are unaffected. |
...
| `SYSTEM_MIGRATION_DELAY_SECONDS`         | `0`         | Delay before executing system migration tasks configured with delay, in seconds. Default is `0` (no delay).                                                                                         |
