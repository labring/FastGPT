...
| `DEFAULT_TEAM_BASIC_PERMISSIONS_ENABLED` | `false`     | Whether newly created teams grant their default all-members group permission to create Apps, Skills, and Datasets. Does not change existing teams. Configure consistently in FastGPT and Pro Admin. |
...
The open-source edition supports custom PDF parsing services, SoMark, TextIn, and Doc2x. Configure only one service. If you configure more than one, AI Platform uses this priority order: custom PDF parsing service, SoMark, TextIn, then Doc2x.
...
2. Add the API key to AI Platform:
...
Restart AI Platform after changing the environment variables. Then enable **Enhanced PDF Parsing** when importing files into a Dataset or configuring App file uploads. PDFs use the configured enhanced parsing service only when this option is enabled; otherwise, AI Platform uses its built-in parser.
