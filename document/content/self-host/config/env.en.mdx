...
| `OPENAI_BASE_URL`        | `https://api.openai.com/v1`         | Legacy setting. No longer used as a default model endpoint fallback when AI Proxy is absent in V4.17.0+.       |
| `CHAT_API_KEY`           | Empty                               | Legacy setting. No longer used as a default credential fallback when the AI Proxy token is absent in V4.17.0+. |
