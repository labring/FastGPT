...
系统插件的远程调试功能套件用于把开发者本地运行的 AI Platform 系统插件临时接入 AI Platform 测试环境。它适合系统插件开发、联调和验收，不适合作为生产插件运行时。

<Alert icon="🤖" context="warning">

系统插件的远程调试功能套件仅商业版支持。

优先推荐在 AI Platform 云服务版本中使用远程调试能力。自部署需要额外维护 Plugin Server、Connection Gateway、Redis、反向代理、TLS 和密钥轮换，运维成本更高。
...
| 组件                 | 作用                                                            |
| -------------------- | --------------------------------------------------------------- |
| AI Platform 主服务       | 提供开启、刷新、关闭调试通道的页面和 API。                      |
...
1. 在 AI Platform 插件调试入口开启调试通道，复制页面返回的连接链接。
2. 在本地插件目录运行：
...
连接成功后，本地 CLI 会通过 Gateway 上报插件 metadata。AI Platform 工具列表中会出现当前调试 source 下的本地插件。调试 source 的格式为：
...
4. 在 AI Platform 中选择调试 source 下的工具并触发一次调用，确认结果由本地插件返回。
