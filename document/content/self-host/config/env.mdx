...
| `PARSE_FILE_WORKER_MEMORY_LIMIT_MB`    | `512`       | 单个文件解析 worker 的 V8 老生代内存上限，单位 MB；最小 `128`。                                |
| `XLSX_PARSE_MAX_ROWS`                  | `100000`    | XLSX 单个工作表允许的最大行数，最小 `1`。                                                      |
| `XLSX_PARSE_MAX_COLUMNS`               | `1000`      | XLSX 单个工作表允许的最大列数，最小 `1`。                                                      |
