...
| 变量                                         | 默认值                   | 说明                                                                                                                                                                                                                 |
| -------------------------------------------- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AGENT_SANDBOX_PROVIDER`                     | 空                       | Agent 沙箱提供方，可选 `sealosdevbox`、`opensandbox`、`e2b`；为空时不启用沙箱。配置该值后，必须同时配置对应 provider 的必填环境变量、`AGENT_SANDBOX_PROXY_SECRET` 和 `AGENT_SANDBOX_PROXY_URL`，否则服务启动会失败。 |
