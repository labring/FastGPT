...
| `AGENT_SANDBOX_SEALOS_IMAGE`                 | 空                       | Sealos Devbox 使用的镜像。                                                                                      |
...
| `AGENT_SANDBOX_VOLUME_MANAGER_URL`           | `http://localhost:3005`  | Volume Manager 服务地址。                                                                                       |
...
| `AGENT_SANDBOX_ARCHIVE_MAX_SIZE`             | `50`                     | Agent 沙箱冷归档包大小上限，单位 MB；用于归档包的上传、下载和打包校验。                                         |
...
| `CHAT_TITLE_MODEL`                     | 空        | 对话标题生成模型；不配置或未命中已启用模型时，回退到系统默认 LLM。                             |
...
这些变量由 `projects/volume-manager/src/env.ts` 加载和校验。FastGPT 侧启用 OpenSandbox 持久化 Volume 时，`AGENT_SANDBOX_VOLUME_MANAGER_TOKEN` 需要与这里的 `VM_AUTH_TOKEN` 保持一致。
...
| `VM_K8S_PVC_STORAGE_SIZE`  | `1Gi`                  | Kubernetes PVC 容量，仅 `kubernetes` 模式需要。         |
