...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `PLUGIN_BASE_URL` | 空 | FastGPT Plugin 服务地址；部署模板通常会配置为内部 Plugin 服务地址。 |
| `PLUGIN_TOKEN` | 空 | 调用 Plugin 服务使用的认证 Token；需与 Plugin 服务配置一致。 |
| `CODE_SANDBOX_URL` | 空 | Code Sandbox 服务地址；部署模板通常会配置为内部 Code Sandbox 服务地址。 |
| `CODE_SANDBOX_TOKEN` | 空 | App 调用 Code Sandbox 时使用的认证 Token，需与沙箱服务 `SANDBOX_TOKEN` 一致。 |
