...
| `CHECK_INTERNAL_IP`                 | `false` | 是否启用内网 IP 检查，用于降低 SSRF 风险。仅作用于用户或工作流可控的出站 URL；部署方配置的服务地址（SSO/外部用户系统、模型地址、飞书/钉钉/语雀基础地址、自定义 PDF 解析、自定义安全校验、快速登录、对话日志、短信代理、发票 Webhook、企业认证等）不受影响。 |
...
| `DEFAULT_TEAM_BASIC_PERMISSIONS_ENABLED` | `false`     | 新建团队时是否为默认全员组授予应用、Skill 和知识库创建权限，不影响已有团队。FastGPT 与 Pro Admin 服务应保持配置一致。 |
...
| `SYSTEM_MIGRATION_DELAY_SECONDS`         | `0`         | 配置了 delay 的系统迁移任务延迟执行时间，单位秒，默认 `0` 为不延迟。                                                  |
