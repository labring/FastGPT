...
| 变量                                             | 默认值                   | 说明                                                                                                                                                                    |
| ------------------------------------------------ | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AGENT_SANDBOX_PROVIDER`                         | 空                       | Agent 沙箱提供方，可选 `sealosdevbox`、`opensandbox`；为空时不启用沙箱。配置该值后，必须同时配置对应 provider 的必填环境变量和三项 Proxy 环境变量，否则服务启动会失败。 |
...
| `AGENT_SANDBOX_PREVIEW_PROXY_URL`                | 空                       | 浏览器访问 Sandbox 文件预览的 HTTP(S) 地址；启用 Agent Sandbox 时必填。建议使用与 FastGPT 主站不同的 origin，避免用户生成的 HTML 脚本进入主站同源安全边界。             |
