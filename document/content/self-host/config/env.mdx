...
- `FILE_TOKEN_KEY`、`AES256_SECRET_KEY`、`INVOKE_TOKEN_SECRET` 与 `JWT_SECRET` 为运行期必填，建议使用随机强密钥，不要使用示例值。
...
| `JWT_SECRET`          | 无，**必填**       | 通用 JWT 签名密钥，长度至少 32 位；FastGPT 与 Pro 必须配置为相同值。                                  |
