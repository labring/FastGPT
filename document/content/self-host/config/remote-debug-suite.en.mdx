...
The system plugin remote debugging suite temporarily connects AI Platform system plugins running on a developer's local machine to a AI Platform test environment. It is intended for system plugin development, integration testing, and acceptance checks, not as a production plugin runtime.

<Alert icon="🤖" context="warning">

The system plugin remote debugging suite is available only in the commercial edition.

We recommend using remote debugging in the AI Platform Cloud version first. Self-hosted deployments require you to operate Plugin Server, Connection Gateway, Redis, reverse proxy, TLS, and secret rotation yourself.
...
| Component            | Purpose                                                                                 |
| -------------------- | --------------------------------------------------------------------------------------- |
| AI Platform main service | Provides the UI and APIs for enabling, refreshing, and revoking a debug channel.        |
...
The AI Platform main service keeps using the regular plugin configuration:
...
1. Enable the debug channel from the AI Platform plugin debug entry and copy the generated connection link.
...
After the connection succeeds, the local CLI reports plugin metadata through Gateway. The local plugins appear in AI Platform under the current debug source. The debug source format is:
...
4. Select a tool under the debug source in AI Platform and invoke it once. The result should come from the local plugin.
