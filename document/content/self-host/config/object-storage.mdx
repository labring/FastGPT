...
这里提供了 AI Platform 目前支持的对象存储厂商，包括自部署的 MinIO、AWS S3、Cloudflare R2、阿里云 OSS 和腾讯云 COS 的环境变量配置说明
...
- 上传固定走 AI Platform 后端代理。
- 对外下载地址固定为 AI Platform 短链，不再直接返回对象存储预签名长链接。
