...
4. Model Mapping: Maps the model name in AI Platform requests to the actual model name at the provider. For example:
...
AI Platform provides simple tests for each model type on the UI to verify that models are working correctly. Each test sends an actual request using a template.
