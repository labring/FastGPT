...
| `AGENT_SANDBOX_OPENSANDBOX_STORAGE_SIZE` | `1Gi`      | OpenSandbox 持久卷容量，仅 K8s 模式下创建新 PVC 时有效。                         |
...
启用 Agent Sandbox 后，必须同时配置 `AGENT_SANDBOX_PROXY_URL` 和 `AGENT_SANDBOX_PREVIEW_PROXY_URL`。前者是浏览器访问 `fastgpt-agent-sandbox-proxy` 的 WebSocket 地址，例如 `wss://sandbox-proxy.example.com`；后者是文件预览使用的 HTTP(S) 地址，例如 `https://sandbox-proxy.example.com`。
