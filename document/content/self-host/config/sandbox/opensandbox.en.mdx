...
`AGENT_SANDBOX_APT_MIRROR` is effective only for root Agent sandboxes. FastGPT writes a separate deb822 source file for the detected Ubuntu or Debian release without replacing the image's existing sources. Ensure that the configured mirror URL is valid and provides the detected distribution's suites. Initialization does not run `apt-get update`.
