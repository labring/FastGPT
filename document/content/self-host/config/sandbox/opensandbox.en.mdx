...
OpenSandbox is designed for self-hosted Agent and Skill sandbox runtimes. AI Platform creates sandboxes through OpenSandbox Server, while Agent Sandbox Proxy provides browser access to files, terminals, and previews.
...
<Alert icon="⚠️" context="warning">
  Host the preview proxy on an origin separate from the AI Platform application, using a different
