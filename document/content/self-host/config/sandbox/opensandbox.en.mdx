...
`AGENT_SANDBOX_APT_MIRROR` is effective only for root Agent sandboxes. FastGPT writes a separate deb822 source file for the detected Ubuntu or Debian release without replacing the image's existing sources. For Debian URLs ending in `/debian`, security updates automatically use the same host's `/debian-security` repository; other URLs do not generate an unverified security suite, so the image's existing security source remains active. Initialization does not run `apt-get update`.
