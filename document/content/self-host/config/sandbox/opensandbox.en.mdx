...
`AGENT_SANDBOX_APT_MIRROR` is handled by the shared sandbox initialization flow and works with both OpenSandbox and Sealos Devbox. FastGPT writes a separate deb822 source file for the detected Ubuntu release without replacing the image's existing sources. If the URL is invalid, the runtime image is not Ubuntu, or the file cannot be written, FastGPT skips the custom source and keeps the original official sources. Initialization does not run `apt-get update`.
