...
对于 root 权限的 Agent 沙箱，FastGPT 会根据 Ubuntu 或 Debian 版本写入一份独立的 deb822 source 文件，不覆盖运行镜像原有 source。请自行保证配置的镜像地址有效，并确保镜像提供对应发行版的 suite。初始化阶段不会执行 `apt-get update`。arm64 运行镜像应配置提供对应架构软件包的镜像地址。
