...
| Variable                                 | Default    | Description                                                                                                                       |
| ---------------------------------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `AGENT_SANDBOX_OPENSANDBOX_CPU_COUNT`    | `1`        | Maximum CPU count for each OpenSandbox instance.                                                                                  |
| `AGENT_SANDBOX_OPENSANDBOX_MEMORY_MIB`   | `2048`     | Maximum memory for each OpenSandbox instance, in MiB.                                                                             |
| `AGENT_SANDBOX_OPENSANDBOX_STORAGE_SIZE` | `1Gi`      | OpenSandbox persistent volume size. Effective only when creating new PVCs in Kubernetes mode.                                     |
...
After Agent Sandbox is enabled, both `AGENT_SANDBOX_PROXY_URL` and `AGENT_SANDBOX_PREVIEW_PROXY_URL` are required. The former is the browser-accessible WebSocket URL for `fastgpt-agent-sandbox-proxy`, such as `wss://sandbox-proxy.example.com`. The latter is the HTTP(S) URL used for file previews, such as `https://sandbox-proxy.example.com`.
