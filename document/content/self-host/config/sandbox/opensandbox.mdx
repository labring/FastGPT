...
OpenSandbox 适合需要自托管 Agent/Skill 沙盒运行环境的场景。AI Platform 通过 OpenSandbox Server 创建沙盒，并通过 Agent Sandbox Proxy 为浏览器提供文件、终端和预览访问能力。
...
<Alert icon="⚠️" context="warning">
  预览代理应部署在与 AI Platform 主站不同的 origin（协议、域名或端口至少一项不同）。Sandbox 中的 HTML
