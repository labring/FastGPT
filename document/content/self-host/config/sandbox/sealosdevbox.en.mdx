...
For root Agent sandboxes, FastGPT writes a separate deb822 source file for the detected Ubuntu or Debian release without replacing the runtime image's existing sources. For Debian URLs ending in `/debian`, security updates automatically use the same host's `/debian-security` repository; other URLs do not generate an unverified security suite, so the runtime image's existing security source remains active. Initialization does not run `apt-get update`.
