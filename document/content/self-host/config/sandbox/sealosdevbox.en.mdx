...
OpenSandbox and Sealos Devbox use the same package registry settings:
...
For apt, FastGPT writes a separate deb822 source file for the detected Ubuntu release without replacing the runtime image's existing sources. If the image is not Ubuntu, the Devbox runtime user cannot write to `/etc/apt`, the URL is invalid, or the file cannot be written, FastGPT skips the custom source and keeps the original official sources. Initialization does not run `apt-get update`.
For arm64 runtime images, configure a mirror URL that provides `ubuntu-ports`.
