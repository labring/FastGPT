...
For root Agent sandboxes, FastGPT writes a separate deb822 source file for the detected Ubuntu or Debian release without replacing the runtime image's existing sources. Ensure that the configured mirror URL is valid and provides the detected distribution's suites. Initialization does not run `apt-get update`.
