...
- **链路追踪**：跟踪用户请求在 AI Platform 各个服务间的完整调用链路
...
变更完成后，等待公网地址就绪，复制该地址，将其填入 AI Platform 中。如果是走内网服务，可以直接复制 4318 端口的内网地址。

![alt text](../../../public/imgs/image-113.png)

## 配置 FastGPT

1. 修改 AI Platform 环境变量
...
Signoz 监控是一个非常占用磁盘的服务，首先不要把 AI Platform debug 日志也存储进来，另外可以将日志存储时长调整为 7 天。如果突然发现 Signoz 数据不增加了，并且内存一直追加，则说明是磁盘满了，需要扩大容量。
