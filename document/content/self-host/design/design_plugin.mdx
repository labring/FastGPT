...
原先 FastGPT 的各项功能均在 FastGPT 的 Next.js 框架内，通过 Monorepo 方式组织。系统插件也曾作为一个子仓库存在于 `FastGPT/packages/plugin` 下。
...
FastGPT Plugin v1.0.0 对插件系统进行了系统性重构，目标是让插件的安装、版本管理、运行隔离和运维配置形成统一模型。
...
系统工具开发结构可以参考 [如何开发系统插件](../../guide/build/tools/system-plugins/dev_system_tool.mdx)。
...
- [FastGPT Plugin](https://github.com/labring/fastgpt-plugin)
- [系统插件开发指南](../../guide/build/tools/system-plugins/dev_system_tool.mdx)
