...
PDF is a relatively complex file format. AI Platform's built-in PDF parser relies on the pdfjs library, which uses logical parsing and cannot effectively handle complex PDF files. When parsing PDFs containing images, tables, formulas, or other non-plain-text content, the results are often poor.
...
For AI Platform versions before V4.9.0, you can use the following method for Marker parsing.
...
Then modify the AI Platform environment variables:
