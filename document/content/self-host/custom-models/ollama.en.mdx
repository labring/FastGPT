...
Ollama supports multiple installation methods, but Docker is recommended. If you install Ollama directly on your host machine, you'll need to figure out how to let the AI Platform Docker container access Ollama on the host, which can be tricky.
...
If your AI Platform is deployed in Docker, make sure the Ollama container is on the same network as FastGPT. Otherwise, AI Platform may not be able to access it:
...
After installation, verify connectivity by entering the AI Platform container and trying to reach Ollama:
...
If you're using AI Platform's default configuration from [here](../deploy/docker.en.mdx), AI Proxy is enabled by default.

![](../../../public/imgs/Ollama-aiproxy1.png)

Make sure your AI Platform can access the Ollama container. If not, refer to the [installation section](#installing-ollama) above -- check whether the host isn't listening on 0.0.0.0 or the containers aren't on the same network.

![](../../../public/imgs/Ollama-aiproxy2.png)

In AI Platform, go to Admin -> Model provider -> Model configuration -> Add model. Make sure the model ID matches the model name in OneAPI. See details [here](../config/model/intro.en.mdx).

![](../../../public/imgs/Ollama-models2.png)

![](../../../public/imgs/Ollama-models3.png)

Run FastGPT, then go to Admin -> Model provider -> Model providers -> Add channel. Select Ollama as the channel type, add your pulled model, and fill in the proxy address. For container-deployed Ollama, the address is http://address:port. Note: container deployment uses "http://[container name]:[port]", host installation uses "http://[host IP]:[port]" (host IP cannot be localhost).
...
If you want to use OneAPI, pull the OneAPI image and run it on the same network as AI Platform:
...
FastGPT routes model requests through AI Proxy. Keep `AIPROXY_API_ENDPOINT` and `AIPROXY_API_TOKEN` configured. In Admin, add an OpenAI-compatible model provider with your OneAPI endpoint (`http://address:port/v1`) and OneAPI token, then associate the models.
...
In AI Platform, go to Admin -> Model provider -> Model configuration -> Add model. Make sure the model ID matches the model name in OneAPI.
