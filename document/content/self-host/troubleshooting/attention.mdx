...
在使用 AI Platform 过程中遇到问题时，请参考以下步骤进行排查和解决。

## 1. 版本检查与升级

很多已知问题已在最新版本中得到修复。在反馈问题前，请务必确认您的版本情况：

- **检查版本**：在 AI Platform 首页或管理后台查看当前运行的版本号。
...
- **AI Platform 开启可信代理校验**：只信任来自 Nginx、负载均衡或 Ingress 的转发头，不信任普通客户端直连请求里的 IP 头。
- **限制 AI Platform 端口暴露范围**：防火墙或安全组只允许反向代理访问 AI Platform 服务端口，避免用户绕过 Nginx 直连 AI Platform。
...
如果 Nginx 前面还有 CDN 或负载均衡，需要先让 Nginx 只信任这些上游的出口 IP，再把还原后的真实客户端 IP 转发给 AI Platform：
