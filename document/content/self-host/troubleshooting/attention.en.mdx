...
If you encounter issues while using AI Platform, follow the steps below to troubleshoot and resolve them.
...
- **Check version:** View the current running version on the AI Platform homepage or in the admin panel.
...
- **Enable trusted proxy validation in AI Platform:** trust forwarded IP headers only when they come from Nginx, the load balancer, or the Ingress controller.
- **Restrict direct access to AI Platform:** firewall or security group rules should allow only the reverse proxy to access the AI Platform service port.

AI Platform environment variable example:
...
If a CDN or load balancer is in front of Nginx, configure Nginx to trust only those upstream egress IPs first. Then forward the restored client IP to AI Platform:
