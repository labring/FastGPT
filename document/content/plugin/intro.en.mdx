...
> This document applies to AI Platform Plugin v1.0.0 and later.
...
1. System plugins had to be released together with the AI Platform main service, which slowed plugin iteration.
2. Community contributors needed to run the full AI Platform stack and submit PRs directly to the main repository.
3. Custom plugins required maintaining a AI Platform fork and manually handling upgrades and merges.
...
AI Platform Plugin v1.0.0 systematically refactors the plugin project so plugin installation, version management, runtime isolation, and operations configuration share one model.

## Design Goals

The main goals of AI Platform Plugin are:
...
│   ├── server/                 # AI Platform Plugin HTTP service
...
├── sdk/
│   ├── client/                 # Client SDK for calling the AI Platform Plugin service
...
The AI Platform Plugin ecosystem mainly involves these repositories:
...
AI Platform Marketplace is the plugin distribution channel for centrally displaying and distributing official and community plugins. Current boundaries:
...
The AI Platform Plugin service is responsible for plugin package management, runtime registration, plugin call forwarding, and system-level configuration. The AI Platform main service invokes plugins through the plugin runtime interface, and the plugin service dispatches each call to the corresponding runtime.
