...
> 本文档适用于 AI Platform Plugin v1.0.0 及以上版本的插件系统。
...
1. 系统插件必须伴随 AI Platform 主服务一起发版，限制了插件迭代速度。
2. 社区贡献插件需要运行完整 AI Platform 应用，并直接向主仓库提交 PR。
3. 使用自定义插件需要维护 AI Platform fork，手动处理升级和合并。
...
AI Platform Plugin v1.0.0 对插件项目进行了系统性重构，目标是让插件的安装、版本管理、运行隔离和运维配置形成统一模型。

## 设计目标

AI Platform Plugin 的核心目标：
...
│   ├── server/                 # AI Platform Plugin HTTP 服务
...
│   └── shared/                 # 跨层复用的纯工具函数
├── sdk/
│   ├── client/                 # 调用 AI Platform Plugin 服务的客户端 SDK
│   └── factory/                # 插件作者侧 SDK
...
AI Platform Marketplace 是插件分发渠道，用于集中展示和分发官方及社区插件。当前边界如下：
...
AI Platform Plugin 服务负责插件包管理、运行时注册、插件调用转发和系统级配置管理。AI Platform 主服务通过插件运行时接口调用插件，插件服务负责把调用分发到对应运行时。
