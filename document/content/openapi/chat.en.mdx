...
- authProxy: object | undefined. API Key auth only. Specifies which team member this request runs as. Supports `username` or `tmbId`; when both are provided, they must resolve to the same member. When continuing with `chatId`, that member must own the chat.
