...
- 使用 APIKey 鉴权。调用 `chat/completions` 时，推荐在请求体传入 `appId`。
- 为兼容 OpenAI SDK，也支持 `Authorization: Bearer <apiKey>-<appId>`。该后缀仅作为传输兼容格式，不会写入数据库。
- 如需通过 `authProxy` 代理团队成员身份，需要团队所有者在创建或编辑该 key 时开启 `authProxy`；代理身份仍需要具备目标应用和会话权限。
