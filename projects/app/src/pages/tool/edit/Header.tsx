...
import { putUpdatePlugin } from '@/web/core/tool/api';
...
type Props = { plugin: ToolItemSchema; onClose: () => void };
...
const Header = ({ plugin, onClose }: Props) => {
  ...
  const flow2ModulesAndCheck = useCallback(async () => {
    ...
    for (let i = 0; i < modules.length; i++) {
      ...
      if (item.flowType === FlowNodeTypeEnum.pluginInput) {
        ...
        if (item.outputs.find((output) => output.targets.length === 0)) {
          toast({
            status: 'warning',
            title: t('module.Tool input must connect')
          });
          return false;
        }
      }
      if (
        ...
        toast({
          status: 'warning',
          title: t('core.module.Tool output must connect')
        });
        return false;
      }
    ...
    if (!pluginInputModule) {
      toast({
        status: 'warning',
        title: t('module.Tool input is required')
      });
      return false;
    }
    if (pluginInputModule.inputs.length < 1) {
      toast({
        status: 'warning',
        title: t('module.Tool input is not value')
      });
      return false;
    }
  ...
  return (
    <>
      ...
      >
        ...
        <MyTooltip label={t('module.Preview Tool')}>
          <IconButton
            mr={[3, 6]}
