...
import { delOnePlugin, postCreatePlugin, putUpdatePlugin } from '@/web/core/tool/api';
...
import { CreateOneToolParams } from '@fastgpt/global/core/tool/controller';
import { customAlphabet } from 'nanoid';
import { MongoImageTypeEnum } from '@fastgpt/global/common/file/image/constants';
const nanoid = customAlphabet('abcdefghijklmnopqrstuvwxyz1234567890', 12);

export type FormType = CreateOneToolParams & {
  id?: string;
};
...
const CreateModal = ({
  ...
  const { mutate: onclickCreate, isLoading: creating } = useRequest({
    ...
    onSuccess(id: string) {
      router.push(`/tool/edit?pluginId=${id}`);
      onSuccess();
      onClose();
    },
  ...
  return (
    <MyModal
      ...
      title={defaultValue.id ? t('plugin.Update Your Tool') : t('plugin.Create Your Tool')}
      isCentered={!isPc}
    >
      <ModalBody>
