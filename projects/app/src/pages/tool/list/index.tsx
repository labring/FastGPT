...
import { getUserPlugins } from '@/web/core/tool/api';
      ...
      >
        {data.map((tool) => (
          <Box
            ...
            onClick={() => router.push(`/tool/edit?pluginId=${tool._id}`)}
          >
            <Flex alignItems={'center'} h={'38px'}>
              <Avatar src={tool.avatar} borderRadius={'md'} w={'28px'} />
              <Box ml={3}>{tool.name}</Box>
              <IconButton
                className="edit"
                position={'absolute'}
                top={4}
