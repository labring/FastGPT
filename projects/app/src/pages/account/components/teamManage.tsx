...
const EditModal = dynamic(() => import('@/components/support/user/team/TeamManageModal/EditModal'));
const InviteModal = dynamic(
  () => import('@/components/support/user/team/TeamManageModal/InviteModal')
);
...
const TeamManage = () => {
  ...
  return (
    <Box display={['block', 'flex']} h={['auto', '100%']} position={'relative'} overflow={'auto'}>
      {/* teams */}
      <Flex
        ...
        <Flex px={5} alignItems={'center'}>
          ...
          {userInfo?.team.role !== TeamMemberRoleEnum.owner && (
            <Button
              ...
              ml={3}
              leftIcon={
                <MyIcon name={'support/account/loginoutLight'} w={'14px'} color={'primary.500'} />
              }
              onClick={() => {
