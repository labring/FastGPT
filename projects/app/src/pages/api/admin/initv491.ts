...
const setupDataset2LinkedList = async () => {
  try {
    // 获取未初始化的集合
    const collections = await MongoDatasetData.distinct('collectionId', { inited: { $ne: true } });

    console.log(`找到 ${collections.length} 个未初始化的集合`);
    ...
    for (const collectionId of collections) {
      try {
        ...
        if (items.length === 0) {
          console.log(`集合 ${collectionId} 没有数据，跳过`);
          // 标记空集合为已初始化
          await MongoDatasetData.updateMany({ collectionId }, { $set: { inited: true } });
          continue;
        }
        ...
        for (let i = 0; i < items.length; i++) {
          try {
            ...
            // 使用单独的更新操作，只设置nextId和inited标记
            await MongoDatasetData.updateOne(
              { _id: currentId },
              { $set: { nextId, inited: true } }
            );
            ...
            console.error(`处理数据项 ${items[i]._id} 时出错:`, itemError);
            // 继续处理下一项
            continue;
          }
        }
        ...
        console.error(`处理集合 ${collectionId} 时出错:`, collectionError);
        // 继续处理下一个集合
        processedCollections++;
        continue;
      }
