...
import { upgradeAppSandboxRuntime } from '@fastgpt/service/core/ai/sandbox/interface/runtime';
import {
  authSandboxSession,
...
async function handler(req: ApiRequestProps): Promise<SandboxRuntimeStatusResponse> {
  ...
  if (sourceType !== ChatSourceTypeEnum.app) {
    throw new Error('Only App sandbox runtime supports chat upgrade');
  }
  ...
  return SandboxRuntimeStatusResponseSchema.parse(await upgradeAppSandboxRuntime(query));
}
