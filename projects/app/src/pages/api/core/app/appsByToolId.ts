...
type Query = { toolId: string };

export type AppsByToolIdItem = {
  _id: string;
  name: string;
  avatar: string;
  intro: string;
...
async function handler(req: ApiRequestProps<unknown, Query>): Promise<AppsByToolIdItem[]> {
  const { toolId } = req.query;

  if (!toolId) {
    return Promise.reject(new Error('toolId is required'));
  }
