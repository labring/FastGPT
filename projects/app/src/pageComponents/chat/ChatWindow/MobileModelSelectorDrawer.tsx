...
const MobileModelSelectorDrawer = ({ isOpen, modelList, value, onChange, onClose }: Props) => {
  ...
  const availableModelList = useMemo(() => {
    ...
    return modelList.filter((model) => myModels.has(model.id) || model.id === value);
  ...
  // Reset the selected provider each time the drawer opens. Adjusted during
  // render (React "adjusting state during rendering" pattern).
