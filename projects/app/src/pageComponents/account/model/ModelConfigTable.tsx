...
const ModelTable = ({ Tab }: { Tab: React.ReactNode }) => {
  ...
  // Root switches scopes; members see all accessible models with server-provided permissions.
  const [activeTab, setActiveTab] = useState<'system' | 'team'>(isRoot ? 'system' : 'team');
  const isSystem = isRoot ? activeTab === 'system' : undefined;
  const modelScopeList = useMemo<{ label: string; value: 'system' | 'team' }[]>(
    () => [
                    ...
                    <Td>
                      <HStack>
                        ...
                        <MyIconButton
                          ...
                          onClick={() =>
                            // F2-S3/F3-S3: model-name second confirmation (design §12);
                            // channel refs hint comes from the list item's channelCount.
                            // Note: no getSystemDefault pre-check here — that applies
                            // to deactivating only (§11.2); delete falls back server-side.
                            openConfirmInput({
                              ...
                              confirmPlaceholder: t('account_model:model.delete_model_placeholder'),
                              confirmValue: item.name || item.model,
