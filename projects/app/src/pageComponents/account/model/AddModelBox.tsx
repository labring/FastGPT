...
import { hasConfiguredModelPrice, sanitizeModelPriceTiers } from '@fastgpt/global/core/ai/pricing';
...
export const AddModelButton = ({
  ...
  if (disabled) {
    return (
      <MyTooltip label={t('config_model:channel_no_permission_tip')}>
        <Box {...buttonBoxProps}>
          <Button {...props} isDisabled>
            {t('config_model:create_model')}
          </Button>
        </Box>
      </MyTooltip>
    );
  ...
  return (
    <MyMenu
      ...
      Button={
        <Box {...buttonBoxProps}>
          <Button {...props}>{t('config_model:create_model')}</Button>
        </Box>
      }
      menuList={[
        {
...
const ChannelAssociateSection = ({
  ...
  return (
    <Section title={t('config_model:model.channel_associate_section')}>
      <Box>
        {!isEditState && (
          <Box fontSize={'xs'} color={'myGray.500'} mb={2}>
            {t('config_model:model.channel_associate_tip')}
          </Box>
        )}
        <Flex alignItems={'center'} justifyContent={'space-between'} mb={2}>
          <Box fontSize={'12px'} fontWeight={'500'} color={'myGray.900'}>
            {t('config_model:model.channel_count')}({matchedChannels.length})
          </Box>
          <Button
            size={'sm'}
            variant={'outline'}
          ...
          >
            {t('config_model:model.quick_create_channel')}
          </Button>
        </Flex>
        {loading ? (
          ...
          <Box color={'myGray.500'} fontSize={'sm'}>
            {t('config_model:model.no_related_channel')}
          </Box>
        )}
        {quickCreating && (
          <EditChannelModal
    ...
    >
      ...
      >
        <Box fontSize={'14px'} fontWeight={'600'} color={'myGray.900'} lineHeight={1.2}>
          {t('config_model:model.template_section')}
        </Box>
        <MyIcon
      ...
      {isExpanded && (
        <Box>
          ...
          >
            <Box fontSize={'12px'} fontWeight={'500'} color={'myGray.900'}>
              {t('config_model:model.template_fill_tip')}
            </Box>
            <Box w={['100%', '220px']}>
              <SearchInput
                bg={'myGray.50'}
                ...
                placeholder={t('config_model:model.template_search_placeholder')}
              />
            </Box>
          </Flex>
          <MyBox isLoading={loading} minH={'120px'}>
            {filteredTemplates.length === 0 && !loading ? (
              <EmptyTip py={6} mt={0} text={t('config_model:model.template_empty')} />
            ) : (
...
export const ModelEditModal = ({
  ...
  const { runAsync: updateModel, loading: updatingModel } = useRequest(
    async (data: SystemModelItemType) => {
      if (data.isSystem !== true) {
        delete data.charsPointsPrice;
        delete data.priceTiers;
        delete data.inputPrice;
        delete data.outputPrice;
      }

      if (data.type === ModelTypeEnum.llm && data.isSystem) {
        const rawPriceTiers = data.priceTiers || [];
        ...
        const hasAnyPrice = hasConfiguredModelPrice({ priceTiers: rawPriceTiers });
        if (!hasAnyPrice) {
          data.priceTiers = [];
        } else {
          const priceTiers = sanitizeModelPriceTiers(rawPriceTiers);
      ...
      try {
        ...
        if (channelCount === 0) {
          toast({ status: 'info', title: t('config_model:model.no_channel_hint') });
        }
      } catch {}
  ...
  return (
    ...
    >
      ...
      <Section key={key} title={t('config_model:model.basic_config_section')}>
        <Flex direction={['column', 'row']} gap={[6, 8]} alignItems={['stretch', 'flex-start']}>
          <Grid flex={'1 0 0'} templateColumns={['1fr', 'repeat(2, minmax(0, 1fr))']} gap={4}>
            ...
            <Field
              label={t('config_model:model.alias')}
              tip={t('config_model:model.alias_tip')}
              required
            >
            ...
            {isRoot && !isEdit && (
              <SwitchField
                label={t('config_model:model.set_system_model')}
                tip={t('config_model:model.set_system_model_tip')}
                field={'isSystem'}
                register={register}
              />
            )}
      ...
      {isLLMModel && (
        <Section title={t('config_model:model.params_config_section')}>
          <Grid templateColumns={['1fr', 'repeat(2, minmax(0, 1fr))']} gap={'16px'}>
            ...
            <Field label={t('config_model:model.max_quote')} required>
              <MyNumberInput
                register={register}
                isRequired
                name="quoteMaxToken"
      ...
      {isEmbeddingModel && (
        <Section title={t('config_model:model.params_config_section')}>
          <Grid templateColumns={['1fr', 'repeat(2, minmax(0, 1fr))']} gap={4}>
            ...
            />
            <Field
              label={t('config_model:model.weight')}
              tip={t('config_model:model.weight_tip')}
              required
            >
              <MyNumberInput
                register={register}
      ...
      {isLLMModel && (
        <Section title={t('config_model:model.feature_config_section')}>
          <Grid templateColumns={['1fr', 'repeat(2, minmax(0, 1fr))']} gap={4}>
            ...
            />
            <SwitchField
              label={t('config_model:model.function_call')}
              tip={t('config_model:model.function_call_tip')}
              field={'functionCall'}
              register={register}
            />
            <SwitchField
      ...
      {priceUnit && feConfigs?.isPlus && isSystemModel && (
