...
export const getWorkflowBuilderToolDisplayName = (
  functionName?: string,
  t?: (key: any) => string
) => {
  const presentation = functionName ? workflowBuilderToolPresentationMap[functionName] : undefined;
  if (!presentation) return;
  if (t) return t(presentation.nameKey);
  if (i18next.isInitialized) {
    const translated = i18next.t(presentation.nameKey);
    if (translated && translated !== presentation.nameKey) return translated;
  }
...
export const getToolDisplayName = ({
  ...
  const builderName = getWorkflowBuilderToolDisplayName(functionName, t);
  if (builderName) return builderName;
  if (!toolName) return '';
  if (t) return t(toolName);
  if (i18next.isInitialized && toolName.includes(':')) {
    const translated = i18next.t(toolName);
    if (translated && translated !== toolName) return translated;
  }
  return toolName;
};
