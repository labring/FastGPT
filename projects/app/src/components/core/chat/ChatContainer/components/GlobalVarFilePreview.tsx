...
const RenderGlobalVarFilePreview = ({
  fileList,
  removeFiles
}: {
  ...
  return fileList.length > 0 ? (
    ...
    >
      {fileList.map((item, index) => {
        ...
        return (
          <MyBox key={index} w={'100%'} h={'36px'} flex={'0 0 auto'}>
            <Box
              border={'sm'}
              boxShadow={
            ...
            >
              ...
              {isFile && (
                <HStack alignItems={'center'} h={'full'}>
                  <MyIcon name={icon as any} w={['1rem', '1.5rem']} h={['1rem', '1.5rem']} />
...
export default React.memo(RenderGlobalVarFilePreview);
