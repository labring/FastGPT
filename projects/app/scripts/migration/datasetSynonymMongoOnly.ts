...
const run = async ({ dryRun, uri, batchSize }: MigrationOptions) => {
  ...
  try {
    ...
    for (const group of configGroups.values()) {
      ...
      if (config.schemaVersion === DatasetSynonymSchemaVersion) {
        const activeVersion =
          typeof config.activeVersion === 'number' ? config.activeVersion : undefined;
        if (activeVersion === undefined || activeVersion < 0) {
          stats.failedDatasets += 1;
          issues.push({ ...issueContext, reason: 'schemaVersion=2 但 activeVersion 无效' });
          continue;
        }
        ...
        const invalidActiveSnapshot =
          (activeVersion > 0 && activeMappings.length === 0) ||
          activeMappings.some(
            (mapping) =>
        ...
        await configCollection.updateOne(
          { _id: configId, schemaVersion: { $ne: DatasetSynonymSchemaVersion } },
          {
            $set: {
              ...
              activeVersion: plan.kind === 'ready' ? LEGACY_VERSION : 0,
              latestVersion: plan.kind === 'ready' ? LEGACY_VERSION : 0,
              updateTime: new Date()
            },
            $unset: {
              pendingVersion: '',
