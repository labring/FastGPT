...
const mockT: TFunction = ((key: string) => {
  const map: Record<string, string> = {
    ...
    'common:core.workflow.check.tool_missing': '该工具不存在或已被删除',
...
describe('checkAppFormResourceIssues', () => {
  ...
  describe('tool checks', () => {
    ...
    ])('should return correct issue message for tool %s error', (error, expected) => {
      const appForm = getDefaultAppForm();
      appForm.selectedTools = [createMockTool({ pluginData: { error } })];
    ...
    it('should return tool_missing when tool is offline', () => {
      ...
      const result = checkAppFormResourceIssues({ appForm, t: mockT });
      expect(result).toBe('该工具不存在或已被删除');
    });
  ...
  describe('dataset and skill resource checks', () => {
    it('should return resource_no_permission when skill has resource_no_permission error', () => {
      const appForm = getDefaultAppForm();
      appForm.selectedAgentSkills = [
        {
          skillId: 'skill-1',
    ...
    it('should return resource_missing when dataset has resource_missing error', () => {
      const appForm = getDefaultAppForm();
      appForm.dataset.datasets = [
        {
          datasetId: 'dataset-1',
