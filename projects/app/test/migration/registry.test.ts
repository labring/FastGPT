...
describe('validateSystemMigrationRegistry', () => {
  it('registers team member role cleanup as the first blocking production task', () => {
    ...
    expect(systemMigrations.slice(2, -3).map((migration) => migration.id)).toEqual([
    ...
    expect(systemMigrations.slice(2, -3).every((migration) => !migration.blockStartup)).toBe(true);
    expect(
      systemMigrations
        .slice(2, -3)
