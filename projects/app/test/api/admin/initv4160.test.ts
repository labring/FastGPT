import handler from '@/pages/api/admin/initv4160';
...
describe('admin/initv4160 (model management migration, additive)', () => {
  ...
  it('migrates apps + app_versions additively: legacy keys kept, canonical added, W1 residue resolved', async () => {
    ...
    await db()!
      ...
      .insertOne({
        ...
        modules: [
          {
            inputs: [
              { key: 'model', value: 'gpt-4o', selectedTypeIndex: 0 },
              // W1 residue：canonical key + name value → 原位解析为 id（不产生重复 canonical）
              { key: 'modelId', value: 'gpt-4o', selectedTypeIndex: 0 },
    ...
    await db()!
      ...
      .insertOne({
        ...
        time: new Date(),
        nodes: [{ inputs: [{ key: 'model', value: 'gpt-4o', selectedTypeIndex: 0 }] }]
      });
    ...
    expect(version.nodes[0].inputs.find((i: any) => i.key === 'modelId').value).toBe(modelId);
  });
