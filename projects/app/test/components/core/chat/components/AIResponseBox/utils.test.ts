...
import { getProcessingPreviewLabel } from '@/components/core/chat/components/AIResponseBox/RenderProcessingPreview';
...
describe('AIResponseBox utils', () => {
  ...
  it('translates Builder tools and labels when t function is provided', () => {
    const mockTranslations: Record<string, string> = {
      'workflow:workflow_builder_tool_query': '查看现状',
      'workflow:workflow_builder_tool_preview': '方案预览',
      'chat:history_generating': '思考中'
    };
    const t = (key: string) => mockTranslations[key] || key;

    expect(getWorkflowBuilderToolDisplayName('workflow_cli_query', t)).toBe('查看现状');
    expect(getWorkflowBuilderToolDisplayName('workflow_builder_present_preview', t)).toBe(
      '方案预览'
    );

    expect(
      getToolDisplayName({
        functionName: 'workflow_builder_present_preview',
        toolName: 'Raw name',
        t
      })
    ).toBe('方案预览');
