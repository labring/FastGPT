...
describe('API Routes', () => {
  ...
  it('GET /sandbox/modules 返回可用模块列表', async () => {
    ...
    const data = await res.json();
    expect(data.success).toBe(true);
    expect(data.data.js).toEqual(env.jsAllowedModules);
    expect(data.data.builtinGlobals).toContain('SystemHelper.httpRequest');
    expect(data.data.python).toEqual(env.pythonAllowedModules);
  });
});
 ...
 */
describe.skipIf(!env.token)('API Auth (requires SANDBOX_TOKEN)', () => {
