...
seccomp_prefix = """
# seccomp disabled by custom sandbox image
...
'''
    ...
    with open(tmp_file, "w", encoding="utf-8") as f:
        f.write(code)
    try:
        result = subprocess.run(["python3", tmp_file], capture_output=True, text=True, timeout=120)
        if result.returncode != 0 and result.returncode < 0:
            return {"error": f"Process terminated by signal: {result.returncode}"}
        if result.stderr != "":
