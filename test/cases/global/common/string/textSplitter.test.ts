...
我们通过一组你是谁的手动数据，对通用模板与问答模板的效果进行对比。此处特意打了个搞笑的答案，通用模板下 GPT35 就变得不那么听话了，`;
  ...
  const { chunks } = splitText2Chunks({ text, chunkSize: 500 });

  // Verify reasonable chunk count (post-processing may merge small chunks)
  expect(chunks.length).toBeGreaterThanOrEqual(4);
  expect(chunks.length).toBeLessThanOrEqual(7);

  // Verify output contains key content sections (post-processing may duplicate headers)
  const outputContent = chunks.join('').replace(/\s+/g, '');
  expect(outputContent).toContain('快速了解FastGPT');
  expect(outputContent).toContain('FastGPT特点');
  expect(outputContent).toContain('引用模板和提示词设计');
...
第二段内容。FastGPT 支持多种 LLM 模型,如 GPT、Claude、文心一言等,未来还将支持自定义的向量模型。其强大的 API 集成能力使得用户可以轻松将其接入现有的 GPT 应用或其他平台,如企业微信、公众号、飞书等。FastGPT 的开源项目遵循 Apache License 2.0 协议,用户可以进行二次开发和发布。,轻松实现复杂的问答场景。补充几个字，hello！！！,轻松实现复杂的问答场景。补充几个字，hello！！！`,
  ...
  // 应该分成 1 个块(post-processing 合并了小块)
  expect(chunks.length).toBe(1);

  // 验证块大于 chunkSize(200),说明使用的是 maxSize
  ...
  expect(chunks[0]).toContain('第二段内容');
  expect(chunks[0]).toContain('FastGPT 支持多种 LLM 模型');
});
...
`;
  ...
  const { chunks } = splitText2Chunks({ text, chunkSize: 300 });

  // Verify reasonable chunk count (post-processing may merge/rearrange chunks)
  ...
  // Verify key content is preserved across all chunks
  const allContent = chunks.join('\n');
  expect(allContent).toContain('测试的呀');
  expect(allContent).toContain('第一个表格');
  expect(allContent).toContain('这是第二段了');
  expect(allContent).toContain('结束了');
  expect(allContent).toContain('张三');
  expect(allContent).toContain('序号22');
});
...
综上,我们预测公司 2024-2026 年,营业收入分别为 187.7/203.3/222.7 亿元, 分别同比增 5.1%/8.3%/9.5%；归母净利润分别为 16.7/19.4/22.6 亿元,分别同比增 $ {0.3}\\% /{15.8}\\% /{16.7}\\% $ ; 每股 EPS 分别为 1.22/1.41/1.65 元; 毛利率分别为 44.5%/45.5%/46.5%。`;
  ...
  // Verify reasonable chunk count (post-processing may change split points via merging and header dedup)
  expect(chunks.length).toBeGreaterThanOrEqual(2);
  ...
  // Verify key content is preserved across all chunks
  const allContent = chunks.join('\n');
  expect(allContent).toContain('关键假设及盈利预测');
  expect(allContent).toContain('心脑血管类');
  expect(allContent).toContain('补益类');
  expect(allContent).toContain('同仁堂经营拆分');
  expect(allContent).toContain('资料来源');
  expect(allContent).toContain('综上');
});
