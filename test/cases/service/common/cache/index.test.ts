...
describe('refreshVersionKey', () => {
  ...
  it('should set a version key without id', async () => {
    ...
    const redis = getGlobalRedisConnection() as any;
    // Verify the key was set by checking the stored value
    const storedValue = await redis.get(`VERSION_KEY:${SystemCacheKeyEnum.systemTool}`);
  ...
  it('should set a version key with specific id', async () => {
    ...
    const redis = getGlobalRedisConnection() as any;
    // Verify the key was set by checking the stored value
    const storedValue = await redis.get(
  ...
  it('should delete all matching keys when id is "*"', async () => {
    // First set some keys so there's something to delete
    const redis = getGlobalRedisConnection() as any;
    await redis.set(`VERSION_KEY:${SystemCacheKeyEnum.systemTool}:key1`, 'val1');
    await redis.set(`VERSION_KEY:${SystemCacheKeyEnum.systemTool}:key2`, 'val2');

    await refreshVersionKey(SystemCacheKeyEnum.systemTool, '*');

    // After deletion with '*', the function should have attempted to find and delete matching keys
    // The mockGetAllKeysByPrefix is mocked to return [] by default, so no actual deletion happens
...
describe('getCachedData', () => {
  ...
  it('should init systemCache if not present', async () => {
    ...
    // Use modelPermission which has a simple default refreshFunc that won't make external calls
