...
// Services (note: dataset service is not available, using direct schema operations)
...
describe('End-to-End Evaluation System', () => {
  ...
  describe('Complete Evaluation Workflow', () => {
    test('应该执行完整的评估工作流', async () => {
      ...
      // 直接创建数据项（因为服务不存在）
      const dataItems = testData.map((item) => ({
        teamId: new Types.ObjectId(teamId),
        tmbId: new Types.ObjectId(tmbId),
      ...
      await MongoEvalDatasetData.insertMany(dataItems);

      // 验证创建成功
      const importedCount = await MongoEvalDatasetData.countDocuments({
        datasetId: new Types.ObjectId(datasetId)
      });
      ...
      // 模拟任务处理器：创建评估项
      const testDataset = await MongoEvalDatasetCollection.findById(datasetId).lean();
      expect(testDataset).toBeTruthy();

      // 获取数据项并创建原子性评估项：每个 dataItem + 每个 evaluator 创建一个评估项
      const testDataItems = await MongoEvalDatasetData.find({
        datasetId: new Types.ObjectId(datasetId)
      }).lean();
      const evalItems: any[] = [];
  ...
  describe('Data Integrity and Edge Cases', () => {
    ...
    test('应该验证评估数据的完整性', async () => {
      ...
      const allDatasets = await MongoEvalDatasetCollection.find({ teamId }).lean();
...
describe('Cleanup and Resource Management', () => {
  test('应该正确清理评估相关资源', async () => {
    ...
    // 创建临时测试数据
    const tempDataset = await MongoEvalDatasetCollection.create({
      teamId: new Types.ObjectId(tempTeamId),
      tmbId: new Types.ObjectId(tempTmbId),
      name: 'Temporary Dataset',
      description: 'Temporary dataset for testing'
    });
    ...
    // 清理资源
    await MongoEvalDatasetCollection.deleteOne({ _id: tempDataset._id });
    await MongoEvalDatasetData.deleteMany({ datasetId: tempDataset._id });
    await EvaluationMetricService.deleteMetric(tempMetric._id, tempAuth);

    // 验证资源已被删除
    const deletedDataset = await MongoEvalDatasetCollection.findById(tempDataset._id);
