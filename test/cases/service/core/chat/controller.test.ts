...
describe('getChatItems', () => {
  ...
  describe('Target DataId Mode', () => {
    it('should return empty array when targetDataId does not exist', async () => {
      await createChatItems(10);
    ...
    it('should fetch items around target message correctly', async () => {
      const items = await createChatItems(10);
      const targetItem = items[5]; // Middle item
      ...
      const result = await getChatItems({
        ...
        limit: 2,
        field: 'obj value',
        targetDataId: targetItem.dataId
      });

      // Should return: 2 prev + target + 2 next = 5 items
      expect(result.histories).toHaveLength(5);
      expect(result.total).toBe(10);

      // Target should be in the middle
      expect(result.histories[2].dataId).toBe(targetItem.dataId);

      // Should be in chronological order
      expect(result.histories[0].value[0].text?.content).toContain('Message 4');
      expect(result.histories[1].value[0].text?.content).toContain('Message 5');
      expect(result.histories[2].value[0].text?.content).toContain('Message 6');
      expect(result.histories[3].value[0].text?.content).toContain('Message 7');
      expect(result.histories[4].value[0].text?.content).toContain('Message 8');
    });

    it('should handle target at the beginning correctly', async () => {
      const items = await createChatItems(10);
      const targetItem = items[0]; // First item
      ...
      // Should return: 0 prev + target + 3 next = 4 items
      expect(result.histories).toHaveLength(4);
      expect(result.histories[0].dataId).toBe(targetItem.dataId);
      expect(result.histories[0].value[0].text?.content).toContain('Message 1');
    });

    it('should handle target at the end correctly', async () => {
      const items = await createChatItems(10);
      const targetItem = items[9]; // Last item
      ...
      // Should return: 3 prev + target + 0 next = 4 items
      expect(result.histories).toHaveLength(4);
      expect(result.histories[3].dataId).toBe(targetItem.dataId);
      expect(result.histories[3].value[0].text?.content).toContain('Message 10');
    });

    it('should handle limit larger than available items', async () => {
      const items = await createChatItems(5);
      const targetItem = items[2]; // Middle item
      ...
      // Should return all items: 2 prev + target + 2 next = 5 items
      expect(result.histories).toHaveLength(5);
      expect(result.histories[2].dataId).toBe(targetItem.dataId);
    });

    it('should respect field projection in target mode', async () => {
      const items = await createChatItems(5);
  ...
  describe('Order Verification', () => {
    ...
    it('should maintain chronological order in target mode', async () => {
      const items = await createChatItems(10);
      ...
      // Verify chronological order - should contain items around target
      expect(result.histories.length).toBeGreaterThan(0);
      // Target should be present
      const hasTarget = result.histories.some((h) => h.dataId === targetItem.dataId);
