...
describe('getMCPChildren', () => {
  ...
  it('should query MongoApp for old MCP format', async () => {
    const appId = '665e5a1b2f3a4b5c6d7e8f90';
    const team2Id = 'aaaaaaaaaaaaaaaaaaaaaaaa';
    const app = {
      _id: appId,
      avatar: '/old-icon.png',
      teamId: team2Id,
    ...
    const findSpy = vi
      .spyOn(MongoApp, 'find')
      .mockReturnValue({ lean: () => Promise.resolve(childApps) } as any);

    const result = await getMCPChildren(app);

    expect(findSpy).toHaveBeenCalledWith({ teamId: team2Id, parentId: appId });
    expect(result).toHaveLength(1);
    expect(result[0]).toMatchObject({
      avatar: '/old-icon.png',
      id: `mcp-${appId}/child_tool`,
      name: 'child_tool'
    });
  ...
  it('should return empty array for old MCP with no children', async () => {
    const appId = '665e5a1b2f3a4b5c6d7e8f91';
    const team3Id = 'bbbbbbbbbbbbbbbbbbbbbbbb';
    const app = {
      _id: appId,
      avatar: '/icon.png',
      teamId: team3Id,
    ...
    const findSpy = vi
      .spyOn(MongoApp, 'find')
      .mockReturnValue({ lean: () => Promise.resolve([]) } as any);
