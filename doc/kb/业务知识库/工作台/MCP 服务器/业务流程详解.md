...
| 用户操作 | 触发 API | 分支条件 | 页面变化 |
|---------|---------|---------|---------|
| 点击页面顶部「创建 MCP 服务器」按钮 | — | 用户无 `hasApikeyCreatePer` 权限 | 按钮置灰不可点击 |
| 点击页面顶部「创建 MCP 服务器」按钮 | — | 用户有 `hasApikeyCreatePer` 权限 | 打开 EditMcpModal 弹窗，模式为创建（isEdit=false），弹窗标题"创建 MCP 服务器" |
...
| 用户操作 | 触发 API | 分支条件 | 页面变化 |
|---------|---------|---------|---------|
| 点击列表行的「编辑」按钮 | — | 用户有 `hasApikeyCreatePer` 权限 | 打开 EditMcpModal 弹窗，模式为编辑（isEdit=true），弹窗标题"编辑 MCP 服务器" |
| 点击列表行的「编辑」按钮 | — | 用户无 `hasApikeyCreatePer` 权限 | 按钮仍可点击（编辑不受 hasApikeyCreatePer 限制） |
