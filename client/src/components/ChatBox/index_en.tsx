import React, {
  ...
  const ChatBox = (
    ...
    // Chat information being generated... Get the current scroll position and determine whether scrolling to the bottom is needed.
    ...
    const sendPrompt = useCallback(
      async (variables: Record<string, any> = {}, inputVal = '') => {
        if (isChatting) {
          toast({
            title: 'The conversation is in progress... Please wait for it to finish',
            status: 'warning'
          });
          return;
        }
        ...
        try {
          ...
          toast({
            title: getErrText(err, 'Chatting error occurred~'),
            status: 'error',
            duration: 5000,
            isClosable: true
          });
