...
const emailMap: { [key: string]: any } = {
  [UserAuthTypeEnum.register]: {
    subject: '注册 AIHelper 账号',
    html: (code: string) => `<div>您正在注册 AIHelper 账号，验证码为：${code}</div>`
  },
  [UserAuthTypeEnum.findPassword]: {
    subject: '修改 AIHelper 密码',
    html: (code: string) => `<div>您正在修改 AIHelper 账号密码，验证码为：${code}</div>`
  }
};
