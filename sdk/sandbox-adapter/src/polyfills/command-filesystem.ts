 ...
 */
export class CommandFilesystemPolyfill {
  ...
  async readFile(path: string): Promise<Uint8Array> {
    try {
      ...
      let totalLength = 0;
      for (let offset = 0; offset < size; offset += CommandFilesystemPolyfill.READ_CHUNK_SIZE) {
        const end = Math.min(offset + CommandFilesystemPolyfill.READ_CHUNK_SIZE, size);
        const chunk = await this.readFileRange({ path, start: offset, end });
        chunks.push(chunk);
   ...
   */
  async readFileRange(props: { path: string; start: number; end?: number }): Promise<Uint8Array> {
   ...
   */
  async appendBytes(props: { path: string; data: Uint8Array; truncate?: boolean }): Promise<void> {
    const { path, data, truncate = false } = props;
    if (truncate) {
  ...
  async deleteDirectories(paths: string[]): Promise<void> {
    for (const path of paths) {
      const result = await this.executor.execute(`rm -rf ${this.shellQuote(path)}`);
      if (result.exitCode !== 0) {
        throw new FileOperationError(
