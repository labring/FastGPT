...
describe('addAuditLog', () => {
  ...
  // rebase 后新引入的组合路径：明细截断与成员名清洗必须同时生效
  it('sanitizes the sentinel inside oversized details before capping', async () => {
    const create = vi.spyOn(MongoTeamAudit, 'create').mockResolvedValue(undefined as never);
    const details = Array.from({ length: 300 }, (_, index) => ({
      resourceId: `resource-${index}`,
      resourceName: index === 0 ? UNSET_TEAM_MEMBER_NAME : `File ${index}`,
      action: 'import',
      result: 'processing'
    }));
    ...
    await addAuditLog({
      ...
      params: { count: '300', details } as never
    });

    const metadata = vi.mocked(create).mock.calls[0][0].metadata as Record<string, any>;
    expect(metadata.details).toHaveLength(200);
    expect(metadata.detailsTruncated).toBe(true);
    // 截断后的明细仍已清洗，不得残留内部哨兵值
    expect(metadata.details[0].resourceName).toBe('');
  });
  ...
  // SYNC_DATASET 走 upsert 补建分支，清洗与截断必须与 create 分支同口径
  it('sanitizes metadata on the sync upsert branch', async () => {
    const updateOne = vi.spyOn(MongoTeamAudit, 'updateOne').mockResolvedValue({
      acknowledged: true,
      matchedCount: 0,
      modifiedCount: 0,
      upsertedCount: 1,
    ...
    await addAuditLog({
      ...
      event: AuditEventEnum.SYNC_DATASET,
      params: {
        datasetName: UNSET_TEAM_MEMBER_NAME,
        result: 'processing',
        taskId: '11111111-1111-4111-8111-111111111111'
      } as never
    });

    const update = vi.mocked(updateOne).mock.calls[0][1] as Record<string, any>;
    expect(update.$setOnInsert.metadata.datasetName).toBe('');
    expect(update.$setOnInsert.scope).toBe('member');
  });
});
