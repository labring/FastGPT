...
describe('isSynonymOperationMongoCommitted', () => {
  it('recognizes a committed version only when every inserted vector is referenced', () => {
    expect(
      isSynonymOperationMongoCommitted({
        currentVersion: 2,
        targetVersion: 2,
        currentVectorIds: ['old', 'new-1', 'new-2'],
        insertedVectorIds: ['new-1', 'new-2']
      })
    ).toBe(true);
    expect(
      isSynonymOperationMongoCommitted({
        currentVersion: 2,
        targetVersion: 2,
        currentVectorIds: ['new-1'],
        insertedVectorIds: ['new-1', 'new-2']
      })
    ).toBe(false);
  });

  it('requires the target version and supports zero-vector commits', () => {
    expect(
      isSynonymOperationMongoCommitted({
        currentVersion: 1,
        targetVersion: 2,
...
describe('normalizeSynonymMappings', () => {
  ...
  it('returns stable fingerprints regardless of synonym input order', () => {
