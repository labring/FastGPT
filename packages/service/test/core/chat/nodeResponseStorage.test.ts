...
describe('createChatItemResponseRows', () => {
  it('flattens child fields into rows and strips nested data', () => {
    ...
    expect(rows.map((row) => row.data.id)).toEqual(['root', 'child', 'grandchild', 'tool-child']);
    expect(rows.map((row) => row.data.parentId)).toEqual([undefined, 'root', 'child', 'root']);
    ...
    expect(rows[0].data.childrenResponses).toBeUndefined();
    expect(rows[0].data.toolDetail).toBeUndefined();
    expect(rows[1].data.childrenResponses).toBeUndefined();
    expect(rows[1].data.quoteList?.[0]).toEqual({
      id: 'quote-1',
      chunkIndex: 0,
      datasetId: 'dataset-1',
      collectionId: 'collection-1',
...
describe('WorkflowNodeResponseWriter', () => {
  ...
  it('flushes rows in batches and releases pending rows after success', async () => {
    ...
    expect(streamedRoot).toHaveLength(2);
    expect(streamedRoot[0]).toMatchObject({
      id: 'root',
      childTotalPoints: 1,
      childResponseCount: 1
    });
    expect(streamedRoot[0].childrenResponses).toBeUndefined();
    expect(streamedRoot[1]).toMatchObject({
      id: 'child',
      parentId: 'root'
    });
  ...
  it('writes slim fallback rows after normal retries keep failing', async () => {
    ...
    expect(create.mock.calls[3][0].map((doc: any) => doc.data.id)).toEqual(['root', 'child']);
    ...
    expect(create.mock.calls[3][0][1]).toMatchObject({
      data: {
        id: 'child',
        parentId: 'root'
      }
    });
    expect(create.mock.calls[3][0][1].data).toEqual({
      nodeId: 'child',
      id: 'child',
      moduleType: FlowNodeTypeEnum.agent,
      moduleName: 'child',
  ...
  it('deletes retry attempt rows from buffer before database flush', async () => {
    const create = vi.fn().mockResolvedValue(undefined);
    const deleteMany = vi.fn().mockResolvedValue(undefined);
    const writer = new WorkflowNodeResponseWriter({
      ...base,
    ...
    await writer.record([makeResponse({ id: 'attempt-0' }), makeResponse({ id: 'attempt-1' })]);
    await writer.deleteResponses(['attempt-0']);
    await writer.close();

    expect(deleteMany).toHaveBeenCalledOnce();
    expect(create.mock.calls[0][0].map((doc: any) => doc.data.id)).toEqual(['attempt-1']);
  });

  it('deletes tracked descendants when cleaning a retry attempt', async () => {
    const create = vi.fn().mockResolvedValue(undefined);
    const deleteMany = vi.fn().mockResolvedValue(undefined);
    const writer = new WorkflowNodeResponseWriter({
      ...base,
    ...
    expect(deleteMany.mock.calls.at(-1)?.[0]).toMatchObject({
      'data.id': { $in: ['attempt-0', 'child', 'grandchild'] }
    });
    expect(writer.getSummary()).toEqual({
      citeCollectionIds: [],
      errorCount: 0,
