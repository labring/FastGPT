...
describe('dispatchParallelRun', () => {
  ...
  it('共用 nodeResponseWriter 时写入任务包装节点，父响应只保留轻量统计', async () => {
    const nodeResponseWriter = {
      ...
      deleteResponses: vi.fn().mockResolvedValue(undefined)
    };
  ...
  it('重试成功时隔离 attempt parent 并清理失败 attempt rows', async () => {
    const nodeResponseWriter = {
      recordWithParent: vi.fn().mockResolvedValue([]),
      deleteResponses: vi.fn().mockResolvedValue(undefined)
    };
    ...
    expect(nodeResponseWriter.deleteResponses).toHaveBeenCalledWith([
      'parallelRun1_task_0_attempt_0',
      'failed-node-response'
    ]);
    expect(nodeResponseWriter.recordWithParent.mock.calls[0][0][0].id).toBe(
      'parallelRun1_task_0_attempt_1'
    );
    expect(result.data[NodeOutputKeyEnum.parallelSuccessResults]).toEqual(['done']);
  });
