...
---`
  ...
  it('injects a global system skill consistently for static and dynamic references', async () => {
    ...
    const dynamicVersions = await injectAgentSkillFilesToSandbox({
      sandbox: sandbox as any,
      skillIds: [String(skill._id)],
      teamId,
      tmbId,
    ...
    expect(dynamicVersions).toEqual(staticVersions);
    expect(dynamicVersions).toEqual([
      {
        skillId: String(skill._id),
        name: 'GlobalSystemSkill',
        description: '',
