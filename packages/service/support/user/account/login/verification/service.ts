...
import {
  ...
  type VerificationConsumeManyContext
} from '../../../../tmpData/verification';
import { maskAccount } from '../../cancellation/formatter';
...
type CreateLoginChallengeParams = {
  userId: string;
  username: string;
  channel: LoginVerificationChannel;
  target: string;
  ...
  try {
    return await withUserLock(material.userId, () =>
      verification.consumeManyInTransaction(
        ...
        async ({ materials, session }: VerificationConsumeManyContext) =>
          handler({
            challenge: materials[0] as LoginChallengeMaterial,
            session
          })
      )
    );
