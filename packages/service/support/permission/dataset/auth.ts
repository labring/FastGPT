...
> {
  ...
  //    视为 owner（含 owner 专属操作），与 listV2/RAG 短路保持一致。
  ...
  if (isTeamOwner) {
    // 团队 owner 短路为 owner：团队 owner 拥有团队内全部资源，与设计「root / team owner →
    // isOwner: true」一致。cap 为 manage 会让「转移 owner」等 owner 专属操作被误拒。
    role = OwnerRoleVal;
