...
export const buildStandardPlan = (
  ...
  maxEvalDatasetAmount: standard?.maxEvalDatasetAmount || standardConstants.maxEvalDatasetAmount,
  maxEvalDatasetDataAmount:
    standard?.maxEvalDatasetDataAmount || standardConstants.maxEvalDatasetDataAmount,
  maxEvalMetricAmount: standard?.maxEvalMetricAmount || standardConstants.maxEvalMetricAmount
});
...
export const getTeamStandPlan = async ({ teamId }: { teamId: string }) => {
  ...
  const plan = standardConstants ? buildStandardPlan(standard, standardConstants) : undefined;

  return {
    [SubTypeEnum.standard]: plan,
    standardConstants: plan
  };
};
