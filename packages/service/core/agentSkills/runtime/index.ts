...
const joinSandboxPath = (basePath: string, path: string) =>
  `${trimSandboxPathRight(basePath)}/${path}`;

const getSkillsRootPath = (workDirectory: string) => joinSandboxPath(workDirectory, 'skills');

// skill 名会进入 sandbox 路径，只保留对目录名安全且可读的部分；真实唯一性由 skillId 保证。
const getSafeSkillDirectoryName = (skillName: string) => {
