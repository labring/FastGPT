...
import type {
  DatasetSearchModeEnum} from '@fastgpt/global/core/dataset/constants';
import {
  SearchScoreTypeEnum
 ...
 */
export async function getAllDatasetsSynonymWords(teamId: string, datasetIds: string[]): Promise<string[]> {
...
export async function embeddingRecall(
  ...
  addLog.debug(`[embeddingRecall] PG returned: indexDataIds=${indexDataIds.length}, collectionIds=${collectionIdList.length}`);
  if (indexDataIds.length > 0) {
    addLog.debug(`[embeddingRecall] sample indexDataIds: ${indexDataIds.slice(0, 3).join(', ')}`);
    addLog.debug(`[embeddingRecall] sample collectionIds: ${collectionIdList.slice(0, 3).join(', ')}`);
  }
  ...
  const [dataMaps, collectionMaps] = await Promise.all([
    MongoDatasetData.find(
      ...
      .then((res) => {
        ...
        addLog.debug(`[embeddingRecall] MongoDB DatasetData: docs=${res.length}, indexMap=${map.size} (pgIds=${indexDataIds.length}, collIds=${collectionIdList.length})`);
        return map;
      }),
...
export async function milvusHybridRecall(options: {
  ...
  const { teamId, datasetIds, queries, model, limit, forbidCollectionIdList, filterCollectionIdList } =
    options;
 ...
 */
export async function mixedRecall(
  options: MixedRecallOptions
): Promise<MixedRecallResult> {
  const {
    teamId,
    datasetIds,
    queries,
