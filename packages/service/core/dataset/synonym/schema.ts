...
import { TeamMemberCollectionName, TeamCollectionName } from '@fastgpt/global/support/user/team/constant';
...
const DatasetSynonymSchema = new Schema(
  {
    ...
    // GridFS 文件ID - 必填，关联到 MongoDB GridFS 中存储的实际文件
    fileId: {
      type: Schema.Types.ObjectId,
      ref: 'dataset.files',
      required: true
 ...
 * 导出 MongoDB 模型
 */
export const MongoDatasetSynonym: Model<DatasetSynonymSchemaType> = getMongoModel<
  DatasetSynonymSchemaType
>(DatasetSynonymCollectionName, DatasetSynonymSchema);
