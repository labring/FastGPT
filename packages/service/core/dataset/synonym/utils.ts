...
/**
 * operation 状态落库失败时，根据主数据版本和向量引用判断 Mongo 是否已经提交。
 * insertedVectorIds 为空时仍要求版本一致，覆盖无需重建向量但需要切换版本的情况。
 */
export const isSynonymOperationMongoCommitted = ({
  currentVersion,
  targetVersion,
  currentVectorIds,
  insertedVectorIds
  ...
  currentVectorIds: string[];
  insertedVectorIds: string[];
}) => {
  if (currentVersion !== targetVersion) return false;
  const currentVectorIdSet = new Set(currentVectorIds);
  return insertedVectorIds.every((id) => currentVectorIdSet.has(id));
};
...
const parseCsvRows = (buffer: Buffer): unknown[][] => {
  ...
  const result = Papa.parse<unknown[]>(text, { skipEmptyLines: false });
...
export const parseSynonymFileRows = ({
  ...
  return rows.slice(1).flatMap((row, index) => {
    ...
    if (!standardizedTerm || synonymTerms.length === 0) return [];
...
export const normalizeSynonymMappings = (
  ...
  return [...mappingMap.values()].map((mapping) => {
    ...
    const fingerprint = createHash('sha256')
      .update(JSON.stringify([mapping.normalizedStandardizedTerm, normalizedSynonymTerms]))
      .digest('hex');
