...
const getAuditResourceId = (event: TrackedAuditEvent, training: RemainingTraining) => {
  if (event === AuditEventEnum.IMPORT_DATASET_CONTENT) return String(training.collectionId);
...
export const refreshTrainingAuditTask = async (auditTaskId?: string): Promise<void> => {
  ...
  try {
    ...
    const expectedCount = Number(audit.metadata?.count ?? audit.metadata?.insertLen ?? 0);
