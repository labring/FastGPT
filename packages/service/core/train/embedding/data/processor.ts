...
> = async (job) => {
  ...
  if (sampledItems.length < trainEnv.TRAIN_MIN_CHUNK_COUNT) {
    addLog.warn('Embedding trainset generation rejected: insufficient chunks after sampling', {
      trainsetId,
    ...
    const error = createEmbeddingEnhancedError(
      null,
      EmbeddingTrainErrEnum.embeddingTrainsetGenInsufficientChunks,
      EmbeddingTrainSuggestionEnum.embeddingTrainsetGenInsufficientChunks
    );
    throw new TrainsetGenerationUnrecoverableError(error);
  }
