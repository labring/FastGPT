...
/**
 * Environment validation for embedding training task creation
 *
...
export async function validateSFTBridgeAccess(): Promise<void> {
  ...
  try {
    ...
    return Promise.reject(EmbeddingTrainErrEnum.embeddingValidationSftBridgeUnaccessible);
  }
}
...
export async function validateDiTingAccess(): Promise<void> {
  ...
  try {
    ...
    return Promise.reject(EmbeddingTrainErrEnum.embeddingValidationDitingUnaccessible);
  }
}
 ...
 *
 * @throws {EmbeddingTrainErrEnum} If any validation fails
 */
