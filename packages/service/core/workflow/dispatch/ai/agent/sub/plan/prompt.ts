...
const getCommonPromptParts = ({
  ...
  \`\`\`
    ...
    bestPractices: `<best_practices>
      ...
      **正确示例**：
      - ✅ "使用 @webSearch 搜索 Python 的学习资源，重点关注官方文档、热门在线课程、入门书籍推荐"
      - ✅ "使用 @webSearch 对比 iPhone 15 和 Samsung S24 的摄像头系统，关注像素、传感器尺寸、夜拍性能"
...
3. **Description 字段撰写规范**
   ...
   **模板**：\`使用 @工具名 [动作][对象]，重点关注[关键方面]\`

   **示例**：
   - ✅ "使用 @webSearch 搜索 iPhone 15 的核心特性，重点关注处理器性能、摄像头系统、续航表现"
   - ❌ "了解 iPhone 15"（太模糊）
   - ❌ "搜索信息"（缺少对象和重点）
  ...
  "steps": [
    {
      ...
      "description": "使用 @webSearch 搜索 Rust 的核心特性，重点关注所有权系统、借用检查器、零成本抽象、并发安全等核心概念"
    },
    {
      "id": "step2",
      "title": "对比 Rust 和 C++ 的性能特性",
      "description": "使用 @webSearch 搜索 Rust 和 C++ 在系统编程中的性能对比，关注内存管理、编译优化、运行时性能基准测试"
    },
    {
      "id": "step3",
  ...
  "steps": [
    {
      ...
      "description": "使用 @webSearch 搜索 FastGPT 的收费标准、免费额度、付费版本价格对比"
    },
    {
      "id": "step4",
      "title": "对比 FastGPT 与 Dify 的差异",
      "description": "使用 @webSearch 搜索 FastGPT 和 Dify 在功能定位、技术架构、易用性方面的对比"
    }
  ]
}
\`\`\`
  ...
  "steps": [
    {
      ...
      "description": "使用 @webSearch 搜索两者的上手难度、学习路径、适合人群、新手友好度对比"
    },
    {
      "id": "step4",
      "title": "对比 React 和 Vue 的生态系统",
      "description": "使用 @webSearch 搜索两者的组件库数量、社区活跃度、企业采用情况、就业市场需求"
    }
  ]
}
\`\`\`
