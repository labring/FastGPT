...
/**
 * Create the post-refactor indexes on system_models and remove the deprecated
 * global model uniqueness index. The latter is matched by name, key and
 * uniqueness before dropping so unrelated customer indexes are preserved.
 */
...
export async function migrateModelData(): Promise<ModelMigrationResult> {
  ...
  logger.info(
    `Model migration (additive): total ${result.total}, flattened ${result.flattened}, isSystem ${result.isSystemSet}, defaults filled ${result.defaultsCleaned}, channel configs collected ${channelConfigs.length}`
  );
...
const migrateInput = (
  ...
  if (input.key && WORKFLOW_MODEL_KEY_MAP[input.key]) {
    ...
    if (!isReference && typeof input.value === 'string' && input.value) {
      const { value, resolved } = convertModelValue(input.value, modelMap);
      if (resolved) {
        // 已有有效 canonical input 不覆盖（含本批 W1 解析已修正的）；否则追加 canonical input
 ...
 * stays idempotent across initv4160 re-runs. If aiproxy is not configured the
...
export async function migrateChannelsFromLegacyConfigs(
  ...
  // step stays idempotent across initv4160 re-runs (design §14).
