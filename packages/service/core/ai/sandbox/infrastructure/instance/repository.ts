...
export type SandboxResourceRef = {
  ...
  lastActiveAt?: Date;
  metadata?: SandboxInstanceSchemaType['metadata'];
  _id?: unknown;
};
...
const getArchiveStartedAt = (resource: SandboxResourceRef | SandboxResourceDoc) => {
  const startedAt = resource.metadata?.archive?.startedAt;
...
export async function findSandboxInstanceBySandboxId(params: {
  ...
  const { provider, sandboxId, status } = params;
...
export async function findSandboxInstanceBySandboxIdAndSource(params: {
  ...
  const { provider, sandboxId, sourceType, sourceId, status } = params;
...
/**
 * 检查指定 provider/sandboxId 是否存在实例记录。
 *
 * 只返回布尔结果，供运行态 session 查询使用；不暴露 Mongo 查询细节给业务层。
 */
export async function existsSandboxInstanceBySandboxId(params: {
  ...
  const { provider, sandboxId } = params;
  return !!(await MongoSandboxInstance.exists({
    provider,
    sandboxId
  }));
}
...
/**
 * 删除远端资源前原子推进到 deleting。
 *
 * 该方法同时承担删除前二次确认和 attempt CAS。只有仍处于同一轮 archiving 且
 * lastActiveAt 未变化的记录才能进入不可逆删除阶段。
 */
export async function tryMarkSandboxDeleting(
  resource: SandboxResourceDoc,
  options: { inactiveBefore?: Date } = {}
) {
  const startedAt = getArchiveStartedAt(resource);
  if (!startedAt) {
    return { matchedCount: 0, modifiedCount: 0 };
  }
  if (options.inactiveBefore && resource.lastActiveAt >= options.inactiveBefore) {
    return { matchedCount: 0, modifiedCount: 0 };
  }
  ...
  return MongoSandboxInstance.updateOne(
    {
      ...
      lastActiveAt: resource.lastActiveAt,
      'metadata.archive.state': 'archiving',
      'metadata.archive.startedAt': startedAt
    },
    {
      $set: {
 ...
 * 标记归档成功。
 *
 * 只有仍处于同一轮 deleting 的记录才能切到 archived。
 * 这里在同一个 Mongo update 中写入当前 provider 的目标 runtime 镜像，保证后续 getStatus/init
 * 都按新 runtime 解释状态。
 */
...
/**
 * 标记归档在进入 deleting 前失败。
 *
 * 失败记录保留在 Mongo 中，方便监控和 Skill Edit 状态识别；后续用户主动升级可从 failed
 * 状态重新抢占为 archiving，普通真实用户运行态也可按策略清理 failed 后继续启动。
 */
export async function markSandboxArchiveFailed(resource: SandboxResourceDoc, error?: string) {
  const startedAt = getArchiveStartedAt(resource);
  if (!startedAt) {
    return { matchedCount: 0, modifiedCount: 0 };
  }
...
export const markSandboxRuntimeUpgradeArchiveFailed = markSandboxArchiveFailed;

/**
 * 进入 deleting 后记录错误但保持不可逆状态，避免业务运行态清理 failed 后绕过恢复。
 */
export async function markSandboxDeletingError(resource: SandboxResourceDoc, error?: string) {
  const startedAt = getArchiveStartedAt(resource);
  if (!startedAt) {
    return { matchedCount: 0, modifiedCount: 0 };
  }
...
/**
 * 清理归档失败标记，让真实用户请求可以重新拉起 sandbox。
 */
export async function clearFailedSandboxArchiveState(resource: SandboxResourceRef) {
  return MongoSandboxInstance.updateOne(
    {
      ...buildSandboxResourceRecordFilter(resource),
      'metadata.archive.state': 'failed'
...
/**
 * 清理节点异常后长时间停留在 archiving 的记录。
 *
 * archiving 尚未确认 S3 包保存完成，超时后只能移除归档标记，允许后续重新抢占。
 */
export async function clearStaleArchivingSandboxStates(startedBefore: Date) {
  return MongoSandboxInstance.updateMany(
    {
      'metadata.archive.state': 'archiving',
...
/**
 * 将长时间停留在 deleting 的记录收敛为 archived。
 *
 * deleting 已经完成 S3 上传，远端删除是否成功不再影响数据恢复；超时后统一按 archived
 * 进入后续 restore，避免节点崩溃或 provider 删除回调丢失导致状态长期卡住。
 */
export async function markStaleDeletingSandboxStatesArchived(startedBefore: Date) {
  let matchedCount = 0;
  let modifiedCount = 0;

  for (const provider of SandboxProviderSchema.options) {
    const result = await MongoSandboxInstance.updateMany(
      {
        provider,
        'metadata.archive.state': 'deleting',
        $or: [
      ...
      },
      {
        $set: {
          ...
          'metadata.archive.state': 'archived',
          'metadata.archive.archivedAt': new Date(),
          ...getCurrentRuntimeImageUpdate(provider)
        },
        $unset: {
          'metadata.archive.deleteStartedAt': '',
          'metadata.archive.failedAt': '',
 ...
 * 原子抢占一个可恢复实例进行恢复。
 *
 * archived 是稳定冷归档态；deleting 也已经有 S3 包，真实用户启动时可以直接从 S3 恢复，
 * 不再等待远端删除收尾。
 */
export async function markSandboxRestoring(resource: SandboxResourceRef) {
  return MongoSandboxInstance.findOneAndUpdate(
    {
      ...buildSandboxResourceRecordFilter(resource),
      'metadata.archive.state': { $in: ['archived', 'deleting'] }
    },
    {
      $set: {
...
/**
 * 按 source/chat 查询单条 sandbox 实例。
 *
 * provider 可选是为了兼容“当前 provider 查询”和“只按业务归属查询”两类场景。
 */
export async function findSandboxInstanceBySourceChat(params: {
  provider?: SandboxProviderType;
  sourceType: ChatSourceTypeEnum;
  sourceId: string;
  chatId: string;
  ...
  const { provider, sourceType, sourceId, chatId, status } = params;
...
/**
 * 按 source/chat 查询资源清理所需的实例记录。
 */
export async function findSandboxResourcesBySourceChat(params: {
  provider?: SandboxProviderType;
  sourceType: ChatSourceTypeEnum;
  sourceId: string;
  chatId: string;
}) {
  const { provider, sourceType, sourceId, chatId } = params;
 ...
 */
export async function findSandboxResourcesBySourceChatExcludeProvider(params: {
  provider: SandboxProviderType;
  sourceType: ChatSourceTypeEnum;
  sourceId: string;
  chatId: string;
 ...
 * 统计某类业务归属下的运行中实例数量。
 *
 * 用于 Skill Edit 等有并发上限的场景；provider 可选，传入时只统计当前 provider。
 */
...
export async function migrateArchivedSandboxInstanceRecord(params: {
  ...
  const { source, provider, sourceType, sourceId, userId, chatId } = params;
  const sourceFilter = {
  ...
  try {
    ...
    type ArchivedSandboxMigrationDoc = Pick<
      SandboxInstanceSchemaType,
      'userId' | 'chatId' | 'metadata' | 'storage' | 'limit'
    > & { _id: unknown };
    ...
    if (!migratedDoc) {
      const existingArchivedTarget = await MongoSandboxInstance.findOneAndUpdate(
        {
          _id: target._id,
          'metadata.archive.state': 'archived'
        },
...
export async function updateSandboxInstanceRecordBySandboxId(params: {
  ...
  const { provider, sandboxId, sourceType, sourceId, userId, chatId, metadata, touchActive } =
    params;
