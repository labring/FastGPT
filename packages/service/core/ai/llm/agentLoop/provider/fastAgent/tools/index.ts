...
export type AgentLoopToolCatalog = {
  ...
  datasetSearchTool?: ChatCompletionTool;
};
...
/**
 * fastAgent 主 loop 的工具可见性。
 * Main Agent 同时看到业务 runtime tools 和 internal tools；内部工具由 fastAgent loop 拦截。
 */
export const getToolsForFastAgentLoop = ({
  ...
  return [
    ...
    ...(normalized.datasetSearchTool ? [normalized.datasetSearchTool] : [])
  ];
};
