 ...
 */
import { askUserToolName } from '../../../systemTools/ask';
import { updatePlanToolName } from '../../../systemTools/plan';
...
不要调用不存在的 runtime tool；如果可以直接回答就直接回答。复杂任务仍可用 ${planToolName} 维护计划，必要时用 ${askToolName} 追问强阻塞信息。
</tool_constraint>`
    : ''
}
...
如果 stop gate 提示不能结束，继续执行或调用 ${updatePlanToolName} 修正状态。
</completion_rules>
