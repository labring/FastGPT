...
export const COMPRESSION_CONFIG = {
   ...
   */
  AGENT_THRESHOLD: 0.55, // 55% 触发压缩
  AGENT_TARGET_RATIO: 0.5, // 压缩到 50%（即原 55% → 27.5%）
   ...
   */
  CHUNK_SIZE_RATIO: 0.4 // 40%（单块不超过此比例）
} as const;
 ...
 *
 * @example
 * const thresholds = calculateCompressionThresholds(100000);
 * // 返回：
 * // {
 * //   dependsOn: { threshold: 15000, target: 12000 },
 * //   agentMessages: { threshold: 55000, target: 27500 },
 * //   singleTool: { threshold: 10000, target: 7000 },
 * //   chunkSize: 40000
 * // }
 */
export const calculateCompressionThresholds = (maxContext: number) => {
  return {
     ...
     */
    agentMessages: {
      threshold: Math.floor(maxContext * COMPRESSION_CONFIG.AGENT_THRESHOLD),
      // 注意：target 是基于当前 token 数计算的，这里返回比例
      targetRatio: COMPRESSION_CONFIG.AGENT_TARGET_RATIO
    },
