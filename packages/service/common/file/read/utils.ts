...
export const readS3FileContentByBuffer = async ({
  ...
  const parsePdfFromCustomService = async (): Promise<ReadFileResponse> => {
    ...
    logger.info('Parsing files from an external service');
    ...
    logger.info(`Custom file parsing is complete, time: ${Date.now() - start}ms`);
  ...
  const start = Date.now();
  logger.debug(`Start parse file`, { body: { extension } });
  ...
  logger.debug(`Parse file success, time: ${Date.now() - start}ms. `);

  // markdown data format
  if (imageList && imageList.length > 0) {
    logger.debug(`Processing ${imageList.length} images from parsed document`);
