/* Vastbase vector crud */
import { DatasetVectorTableName } from '../constants';
import { delay, retryFn } from '@fastgpt/global/common/system/utils';
import { VastbaseClient, connectVastbase } from './controller';
import { type VastbaseSearchRawType } from '@fastgpt/global/core/dataset/api';
import type {
...
export class VastbaseVectorCtrl {
  constructor() {}
  init = async () => {
    try {
      await connectVastbase();
      await VastbaseClient.query(`
        CREATE TABLE IF NOT EXISTS ${DatasetVectorTableName} (
            id BIGSERIAL PRIMARY KEY,
            vector FLOATVECTOR(1536) NOT NULL,
      ...
      await VastbaseClient.query(
        `CREATE INDEX CONCURRENTLY IF NOT EXISTS vector_index ON ${DatasetVectorTableName} USING hnsw (vector floatvector_ip_ops) WITH (m = 32, ef_construction = 128);`
      );
      await VastbaseClient.query(
      ...
      addLog.info('init Vastbase successful');
    } catch (error) {
      addLog.error('init Vastbase error', error);
    }
  };
  insert = async (props: InsertVectorControllerProps): Promise<{ insertIds: string[] }> => {
    ...
    const { rowCount, rows } = await VastbaseClient.insert(DatasetVectorTableName, {
      values
    });
  ...
  delete = async (props: DelDatasetVectorCtrlProps): Promise<any> => {
    ...
    await VastbaseClient.delete(DatasetVectorTableName, {
      where: [where]
    });
  };
  embRecall = async (props: EmbeddingRecallCtrlProps): Promise<EmbeddingRecallResponse> => {
    ...
    const results: any = await VastbaseClient.query(
      `BEGIN;
    ...
    const rows = results?.[results.length - 2]?.rows as VastbaseSearchRawType[];
  ...
  getVectorDataByTime = async (start: Date, end: Date) => {
    const { rows } = await VastbaseClient.query<{
      id: string;
      team_id: string;
      dataset_id: string;
  ...
  getVectorCountByTeamId = async (teamId: string) => {
    const total = await VastbaseClient.count(DatasetVectorTableName, {
