...
export const AGENT_TOOLS: ToolDefinitionItem[] = [
  {
    name: TOOLS.SEARCH,
    description:
      'Search the knowledge base for relevant information. Use this to find documents that can help answer the user question.',
    inputSchema: {
      type: 'object',
      properties: {
...
export function parseToolCallXml(
  ...
  // 使用 stripThinkBlocks 对齐 Python strip_think_blocks
  const cleaned = stripThinkBlocks(content);
  const regex = /<tool_call>\s*<tool name="(\w+)"[^>]*>([\s\S]*?)<\/tool>\s*<\/tool_call>/g;
  let match;
...
function buildAnswerSystemPrompt(playbookContent: string): string {
  return `You are DiTing, an enterprise knowledge base QA assistant. You answer questions \
...
**STOP. Before calling @answer, you MUST gather sufficient information. Use @query_rewrite to optimize the query if needed, then @search to retrieve chunks.**
...
- You MUST use @answer to generate the final answer.
- Only cite chunks returned by @search in the CURRENT turn.

If you answer without gathering sufficient information first, you are violating this law.
...
|------|---------|-----------------|
| **Direct** | Focused, single-fact question | One search, then answer |
...
loop {
  ...
  (3) Act: call @search, @query_rewrite, or @answer
...
When you have sufficient information, call @answer. Before calling, confirm:
...
If @answer's reflection result is unfaithful or incomplete:
- Search for the missing information, then call @answer again.

## Constraints

- Total tool calls ≤ 10
- @search calls ≤ 5
- Always use @answer to generate the final response (do NOT write the answer yourself)
- If repeated searches find nothing relevant, call @answer to produce an honest \
...
- @answer({"reasoning": "..."}) — generate final answer from retrieved chunks
...
**GLOBAL RULE: Do NOT call @answer. When your searches are complete, simply stop (make no more tool calls).**
