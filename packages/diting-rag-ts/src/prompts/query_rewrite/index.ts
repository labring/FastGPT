// src/prompts/query_rewrite/index.ts
// Query Rewrite 策略定义（对齐 Python 版）
...
/**
 * 旧版兼容类型（用于 prompt_loader.ts）
 */
export interface RewriteStrategyDef {
  name: string;
  description: string;
}

/**
 * 默认策略规范（6种，对齐 Python 版）
 */
...
/**
 * 旧版兼容导出（用于 prompt_loader.ts）
 */
export const rewriteStrategies: Record<string, RewriteStrategyDef> = {
  cce: { name: 'cce', description: 'Core Content Extraction' },
  decompose: { name: 'decompose', description: 'Query Decomposition' },
...
/**
 * 旧版兼容 prompt（用于 prompt_loader.ts）
 * 使用 DEFAULT_STRATEGIES 中的 prompt_template
 */
export const rewriteStrategyPrompts: Record<string, string> = DEFAULT_STRATEGIES.reduce(
  (acc, spec) => {
    acc[spec.name] = spec.prompt_template;
    return acc;
  },
...
/**
 * 策略选择 Prompt（对齐 Python STRATEGY_SELECTION_PROMPT）
 */
export const STRATEGY_SELECTION_PROMPT = `You are a query analysis expert. Based on the characteristics of the user's query, \\
...
/**
 * 批量改写 Prompt（对齐 Python BATCH_REWRITE_PROMPT）
 */
export const BATCH_REWRITE_PROMPT = `Rewrite the following query according to the specified strategies. \\
...
/**
 * 将策略格式化为选择 Prompt 的一部分
 */
export function formatStrategyDescriptions(strategies: RewriteStrategySpec[]): string {
  const enabled = strategies.filter((s) => s.enabled).sort((a, b) => b.priority - a.priority);
  const lines: string[] = [];
...
/**
 * 获取策略的重写指令（带 few-shot examples）
 */
export function getRewriteInstructions(strategies: RewriteStrategySpec[], query: string): string {
  const instructions: string[] = [];
  for (let i = 0; i < strategies.length; i++) {
    const spec = strategies[i];
    let exampleStr = '';
