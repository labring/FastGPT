...
- \`detail=false, stream=true\`：返回兼容 GPT 的 SSE 流；未命名会话标题生成成功时会额外返回 \`chatTitle\` event。
...
**event 取值**（\`stream=true\` 场景下，除 \`chatTitle\` 外，通常 \`detail=true\` 才会有非 answer 的 event）
...
兼容 GPT 的 SSE 流。回答 chunk 仅包含 \`data\`（无 \`event\`），未命名会话标题生成成功时会以 \`chatTitle\` event 返回：
