...
import { ChatSourceEnum, FeedbackFilterEnum } from '../../../../core/chat/constants';
...
export const ChatLogItemSchema = z.object({
  ...
  correctionCount: z.int().nullish().meta({ example: 0, description: '优化回答数' }),
...
export const GetAppChatLogsBodySchema = PaginationSchema.extend(
  ...
  feedbackFilter: z
    .array(z.enum(FeedbackFilterEnum))
    .optional()
    .meta({
      example: [FeedbackFilterEnum.good, FeedbackFilterEnum.bad],
      description: '反馈筛选：good-好评，bad-差评，noFeedback-无反馈'
    })
});
