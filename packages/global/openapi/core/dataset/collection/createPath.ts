...
export const DatasetCollectionCreatePath: OpenAPIPath = {
  ...
  '/core/dataset/collection/create/backup': {
    post: {
      ...
      description:
        '上传表头由 q、a、index、metadata 组成的 CSV 备份文件，恢复数据到知识库集合。metadata 单元格为 JSON object。`file` 为 CSV 文件，`data` 为 JSON 序列化的集合参数对象',
      tags: [DevApiTagsMap.datasetCollectionCrteate],
      requestBody: {
        content: {
          'multipart/form-data': {
  ...
  '/core/dataset/collection/create/template': {
    post: {
      ...
      description:
        '上传表头由 q、a、index、metadata 组成的 CSV 模板文件，批量导入数据到知识库集合。metadata 单元格为 JSON object。`file` 为 CSV 文件，`data` 为 JSON 序列化的集合参数对象',
      tags: [DevApiTagsMap.datasetCollectionCrteate],
      requestBody: {
        content: {
          'multipart/form-data': {
