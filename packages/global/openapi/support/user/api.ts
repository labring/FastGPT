...
/** 用户体系列表接口共用的分页请求参数。 */
export const UserPaginationBodySchema = z
  .object({
    pageSize: IntSchema.positive().optional().meta({
      example: 20,
      description: '每页条数，默认 10'
    }),
    pageNum: IntSchema.positive().optional().meta({
      example: 1,
      description: '页码，从 1 开始；与 offset 二选一'
    }),
    offset: IntSchema.optional().meta({
      example: 0,
      description: '偏移量；与 pageNum 二选一'
    })
  })
  .meta({
    description: '用户体系列表分页参数'
  });
export type UserPaginationBodyType = z.infer<typeof UserPaginationBodySchema>;

/* ============================================================================
 * API: 搜索团队成员、组织和用户组
 * Route: GET /api/proApi/support/user/search
 * Method: GET
...
export const SearchUserResponseSchema = z
  .object({
...
/* ============================================================================
 * API: 同步用户和组织
 * Route: POST /api/proApi/support/user/sync
 * Method: POST
