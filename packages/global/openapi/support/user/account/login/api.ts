...
export const LoginVerificationChannelSchema = z.enum(['email', 'phone']).meta({
  description: '二次验证渠道',
  example: 'email'
});
export type LoginVerificationChannel = z.infer<typeof LoginVerificationChannelSchema>;
...
export const LoginVerificationRequiredResponseSchema = z
  .object({
    ...
    channel: LoginVerificationChannelSchema,
...
export const LoginVerificationResolveResponseSchema = z.discriminatedUnion('status', [
  z
    .object({
      ...
      accountKind: z.enum(['email', 'phone']),
  ...
  z
    .object({
      ...
      unsupportedReason: z.enum(['empty_username', 'no_available_verification_method'])
    })
    .strict()
]);
