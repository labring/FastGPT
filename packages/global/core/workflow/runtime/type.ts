...
import { DatasetSearchModeEnum, DatasetRetrievalModeEnum } from '../../dataset/constants';
  ...
  .object({
    ...
    queryExtensionResult: z
      .object({
        ...
        synonymRewriteResult: z
          .object({
            standardizedQuery: z.string().meta({ description: '标准化查询' }),
            coreferenceResolved: z.string().meta({ description: '指代消除后的查询' })
          })
          .optional()
          .meta({ description: '同义词改写结果' }),
        rewriteTime: z.number().optional().meta({ description: '改写耗时（秒）' })
      })
      .optional()
    ...
    retrievalType: z
      .enum(['correction', 'faq'])
      .optional()
      .meta({ description: '检索类型标记，仅当 correction 或 faq 命中时存在' }),
    retrievalResults: z
      .array(SearchDataResponseItemSchema)
      .optional()
    ...
    rerankError: z
      .object({
        ...
        i18nErrorMessageData: z.record(z.string(), z.any()).optional()
      })
      .optional()
      .meta({ description: '重排错误信息' }),
    correctSearchResult: z.any().optional().meta({ description: '纠正搜索结果' }),
