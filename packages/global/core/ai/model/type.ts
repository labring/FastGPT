...
export const ModelPriceTierSchema = z
  .object({
    minInputTokens: z.number().min(0).optional().meta({
      description: 'Minimum input tokens, unit: k/tokens'
    }),
    maxInputTokens: z.number().min(0).nullish().meta({
      description: 'Maximum input tokens, unit: k/tokens. If not provided, treated as unlimited.'
    }),
    inputPrice: z.number(),
    outputPrice: z.number()
  })
  .meta({
    description: 'Model price tier, left-open right-closed interval.'
  });
...
export const BaseModelItemSchema = z.object({
  // _id.toString() — always set by cache layer on load
  id: z.string(),
  provider: z.string(),
  model: z.string(),
  name: z.string(),
  ...
  isSystem: z.boolean().optional(), // replaces isCustom (semantic reversal)

  // tmbId and teamId only stored for private models (isSystem: false)
  tmbId: z.string().optional(),
  teamId: z.string().optional(),

  // Note: requestUrl, requestAuth removed — managed by Channel
  // Note: isDefault removed — managed by default_models table
...
export const LLMModelItemSchema = PriceTypeSchema.extend(BaseModelItemSchema.shape).extend({
  ...
  // Note: isDefaultDatasetTextModel, isDefaultDatasetImageModel, isDefaultChatTitleModel removed
  // — managed by default_models table
