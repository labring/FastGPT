...
/**
 * 可接收验证码的账号联系方式渠道，取值来自账号类型中的邮箱和手机号。
 * 登录 Challenge 材料、主服务与 Pro 之间的登录二次验证接口都复用这一份声明，
 * 渠道集合只需在此处调整；从账号类型枚举 extract 而非重新字面量声明，保证两处取值不会漂移。
 */
export const AccountContactChannelSchema = RecognizedAccountKindSchema.extract([
  'email',
  'phone'
]).meta({ description: '验证码投递渠道', example: 'email' });
export type AccountContactChannel = z.infer<typeof AccountContactChannelSchema>;
