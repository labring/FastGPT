...
import {
  ...
  normalizeWorkflowToolInputDefaultMode
...
describe('workflowTool utils', () => {
  describe('normalizeWorkflowToolInputDefaultMode', () => {
    const createInput = (input: Partial<FlowNodeInputItemType> = {}) =>
      ({
        key: 'query',
        label: 'Query',
        valueType: 'string',
    ...
    it('migrates legacy workflow tool descriptions to the AI generated default', () => {
      expect(
        normalizeWorkflowToolInputDefaultMode(
          createInput({ toolDescription: 'Query generated by AI' })
        )
      ).toMatchObject({ isToolParam: true });
    });

    it('keeps legacy manual inputs without tool descriptions unchanged', () => {
      expect(normalizeWorkflowToolInputDefaultMode(createInput())).not.toHaveProperty(
        'isToolParam'
      );
    });

    it('preserves an explicit new-protocol default', () => {
      expect(
        normalizeWorkflowToolInputDefaultMode(
          createInput({ toolDescription: 'Query description', isToolParam: false })
        )
      ).toMatchObject({ isToolParam: false });
    });

    it('does not enable AI generation for unsupported workflow inputs', () => {
      expect(
        normalizeWorkflowToolInputDefaultMode(
          createInput({
            toolDescription: 'Dataset selector',
            renderTypeList: [FlowNodeInputTypeEnum.selectDataset]
          })
        )
      ).not.toHaveProperty('isToolParam');
    });
  });
