...
describe('builtinTemplateProvider', () => {
  ...
  it('publishes value schemas for common complex PR2 parameters', async () => {
    ...
    expect(descriptor.inputs.find((input) => input.key === 'system_addInputParam')).toBeUndefined();
...
describe('normalizeNodeTemplateDescriptor', () => {
  it('normalizes current template fields and automation metadata', async () => {
    ...
    const modelInput = descriptor.inputs.find((input) => input.key === NodeInputKeyEnum.aiModelId);
  ...
  it('uses validated remote defaults and clears unverified resource template values', async () => {
    ...
    const provider = {
      ...
      resolve: async () => {
        const resolved = await builtinTemplateProvider.resolve(ref, { locale: 'en' });
        const template = structuredClone(resolved.template);
        template.inputs.find((input) => input.key === NodeInputKeyEnum.aiModelId)!.value =
          'template-model';
        return {
          ...resolved,
          template,
          validatedInputDefaults: {
            [NodeInputKeyEnum.aiModelId]: { provided: true as const, value: 'validated-model' }
          }
        };
      }
    };
    ...
    expect(
      node.node.inputs.find((input) => input.key === NodeInputKeyEnum.aiModelId)
    ).toMatchObject({
    ...
    const localModel = localNode.node.inputs.find(
