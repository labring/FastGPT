...
import type {
  UserDocument,
  UserMongooseSchemaType
} from '../../../../mongodb/business/support/user/schema';
...
describe('MongoUserRepository.findByCredentials', () => {
  it('loads the stored password hash and compares it outside the query filter', async () => {
    const { repository, model, queries } = createRepository();

    const user = await repository.findByCredentials({
      username: 'user@example.com',
      password: 'hashed-password'
    });

    expect(model.findOne).toHaveBeenCalledWith({ username: 'user@example.com' });
    expect(queries.findOne.select).toHaveBeenCalledWith('+password');
    expect(user?.id).toBe(userId);
  });
  ...
  it('supports credential lookup by domain id', async () => {
    ...
    await repository.findByCredentials({ id: userId, password: 'hashed-password' });

    expect(model.findOne).toHaveBeenCalledWith({ _id: new Types.ObjectId(userId) });
  });
