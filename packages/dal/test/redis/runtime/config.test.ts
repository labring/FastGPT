...
import { parseRedisConnectionConfig, RedisConfigurationError } from '@fastgpt/dal/redis/runtime';
...
describe('parseRedisConnectionConfig', () => {
  ...
  it.each([
    ...
    ['redis:///0', 'REDIS_URL host must not be empty'],
  ...
  it('strictly rejects non-string input without coercion', () => {
    expect(() => parseRedisConnectionConfig(6379 as any)).toThrow(
      new RedisConfigurationError('REDIS_URL must be a string')
    );
  });
