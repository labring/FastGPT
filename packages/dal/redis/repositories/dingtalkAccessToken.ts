...
export type DingtalkAccessTokenRepositoryLogger = {
  warn: (message: string, metadata: Record<string, unknown>) => void;
};

export type DingtalkAccessTokenRepositoryDependencies = {
  redis?: Pick<RedisStoreAdapter, 'delete' | 'get' | 'set'>;
  logger: DingtalkAccessTokenRepositoryLogger;
};

/**
 * 创建 Dingtalk access token Repository。
 *
 * Repository 保持历史物理 key、动态 TTL 和进程内 single-flight。Redis 读写失败按缓存
 * miss 降级，不阻断上游 token 获取；上游错误仍原样抛给调用方。
 */
export const createDingtalkAccessTokenRepository = ({
    ...
    getOrRefresh: async ({
      ...
      const refreshPromise = (async () => {
        try {
          ...
          await redis.delete(cacheKey).catch(() => undefined);
          throw error;
        } finally {
...
export type DingtalkAccessTokenRepository = ReturnType<typeof createDingtalkAccessTokenRepository>;
