...
export type WechatPollingFailureRepositoryDependencies = {
  redis?: Pick<RedisStoreAdapter, 'delete' | 'incrementIntegerWithTtl' | 'set'>;
};
...
/**
 * 创建 Wechat polling failure counter Repository。
 *
 * 计数递增由 adapter 以 INCRBY + EXPIRE NX 事务完成，避免多个 worker 读取后写回时丢失更新。
 * Redis 错误不在 Repository 内吞掉，worker 继续沿用 failed/退避语义。
 */
export const createWechatPollingFailureRepository = ({
  ...
  clear: (shareId: string) => redis.delete(getWechatPollingFailureKey(shareId))
});

export type WechatPollingFailureRepository = ReturnType<
  typeof createWechatPollingFailureRepository
>;

export const wechatPollingFailureRepository = createWechatPollingFailureRepository();
