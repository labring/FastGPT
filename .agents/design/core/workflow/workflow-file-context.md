...
query、histories 和全局文件变量，并允许交互恢复入口及 Plugin Input 登记 `fileSelect` 文件。下游消费者
...
- 文件变量一旦由节点 `set()` 更新，只接受绝对 HTTP(S) URL，并统一按外链存储，不再还原私有 key；
- 交互恢复入口只登记当前表单配置为 `fileSelect` 的值；
- Plugin Input 只登记节点配置为 `fileSelect` 的字段，同时兼容文件对象数组和 URL 字符串数组；
...
SSRF-safe reader，并使用同一请求级 `maxFileSize`。
...
`fileSelect` 输入派生最小 Context：父 Context 命中的文件继承可信 Ref 和原 `modelUrl`，不重新
签名；父 Context 未命中的合法绝对 HTTP(S) URL 在 Child 内登记为外链。Child 不得根据签名
URL 结构反解私有 key，也不能通过派生 Context 枚举未选择的父文件。若父签名 URL 作为普通
字符串显式传给 Child，则它本身是 bearer URL，按已传入的绝对外链处理，不属于 Context 隔离范围。
...
- 节点更新文件变量时只接受绝对 HTTP(S) URL，并将原内部文件 URL 转为外链存储；
...
- Child 只在派生 Context 中继承实际传入的父 Ref，同一私有文件不重复签名；
- 作为普通字符串显式传入 Child 的签名 URL 按 bearer 外链处理；
