...
- 文件变量更新数量优先使用该变量配置的 `maxFiles`，未配置时回退到请求级
  `WorkflowFileContext.limits.maxFileAmount`，两者都不存在时默认为 5；该值在
  `WorkflowVariableState.create` 登记变量时归一化，后续更新只读取确定的 `config.maxFiles`；
...
`global.feConfigs.uploadFileMaxAmount`。该值用于限制 Workflow 中动态读取和未单独配置
`maxFiles` 的文件输入，不受应用 query 上传配置影响。
