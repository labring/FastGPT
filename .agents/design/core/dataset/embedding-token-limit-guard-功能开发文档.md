...
本次优化目标是：不重写原有知识库分块策略，只保证最终进入 embedding 的文本不超过模型 `maxToken`。
...
- 不改变前端交互、API 入参、向量库结构。
- 不做存量数据迁移。
- 不专项优化 Markdown 表格、代码块、PDF 等内容的语义分块质量。
...
packages/service/worker/text2Chunks/split.ts
```

职责：

- `splitText2Chunks` 放在 `packages/service`，前端导入预览通过后端接口调用，不再直接依赖分块实现。
- `splitText2Chunks` 新增可选 `lengthOptions`，允许调用方注入长度计数器和超长文本切分函数。
- 默认不传 `lengthOptions` 时仍按原字符长度逻辑执行，前端和原文分块行为不变。
- worker 层 `splitText2ChunksByLengthUnit` 在 `lengthUnit = 'token'` 时注入 `countPromptTokensInWorker` 和 token-safe 切分函数。

边界：

- 不把 worker tokenizer 放到 `packages/global`，避免 global 包承担后端运行时依赖。
...
原始 q/a/index text
-> 先走原来的 text2Chunks
-> 得到原有分块语义下的 index chunk
-> index 分块阶段调用 text2Chunks 的 token 模式
-> 超长 index 按 embedding maxToken 拆成多条 index
-> 再写入向量库
```
...
- `getSystemIndexes`：系统默认索引会先按原逻辑生成，再做 token-safe 拆分。
- `formatIndexes`：外部传入的文本索引也会做同样的 token-safe 处理。
...
getVectors inputs
-> 校验输入结构和空输入
-> text input 按 model.maxToken 截断
...
- 具体 provider 的 tokenizer 可能与 FastGPT 统一 token worker 存在差异；如果 provider 仍返回超限错误，需要回到对应上游入口补处理逻辑。
...
入库阶段允许拆分为多条 index；检索和相似度阶段只截断，不扩增 query 数量；`getVectors` 不再做额外 token 预判。
