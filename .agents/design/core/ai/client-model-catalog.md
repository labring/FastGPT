...
状态：2026-09-08 就近读取改造已实现并通过局部自动验证，待页面验收；下方历史设计以本节最新决策为准。

## 2026-09-08 已确认：按业务用途就近读取

### 需求与边界

- catalog 是当前身份完整模型数据的唯一来源；缓存、版本、持久化和在途请求集中管理，但不在顶层主动加载。
- 每个业务在实际使用处自行调用 summary/detail/list/default；禁止父页面或 Context 为子业务预加载 catalog，再通过 Context/props 下发模型列表或详情。
- getModelSummary/useModelSummary 优先读取当前身份 catalog，再读摘要缓存，未命中才请求轻量摘要接口；空 ID 不请求。
- getModelDetail/useModelDetail 异步确保当前身份 catalog 就绪，再精确查找已选模型。需要 token、音色、视觉能力的业务自行使用此入口。
- getModelList/useModelList 用于候选选择等真正需要列表的业务，选择器展开才加载。每次重新启用、切换查询或刷新都建立新的请求代次，旧目录不能被标记为本次加载成功。
- Agent 知识库参数直接从节点 inputs 派生，按字段存在性恢复，包括当前为空的模型 ID；弹窗内部维护编辑草稿。
- 输出能力由始终挂载的 NodeCard 调用节点级 hook 计算，RenderOutput 只展示。模型加载或请求失败时保留原状态，回写前检查节点及输入未变化。
- getModelDefault 异步复用 catalog：业务默认优先，再用 catalog 已计算的有效默认，额外条件不满足时选第一个符合条件的候选；无候选返回空。chatTitleLLM 保持无自动回退。
- 数据读取不修改表单；新建或显式开启功能的业务自行获取默认并赋值，返回时检查用户值、功能开关和身份未变化。非空失效引用不得自动替换。
- 历史名称解析独立于默认赋值；快照恢复、撤销和展示不补默认。
- detail 不新增服务端接口；现有展示 detail 接口命名收敛为 summary，保留现有鉴权和客户端安全字段白名单。
- catalog 未加载、请求失败、完整目录未命中是不同状态；不得将缓存未命中当作下架，不得用默认模型能力掩盖实际选择。
- 不改变服务端运行降级、发布模型政策、模型协作者权限边界和迁移任务；管理员、公开价格、团队权限列表保持独立。

### 开发与验证 TODO

- [x] 统一 catalog 就绪、身份隔离及异步 getModelDetail/getModelList/getModelDefault。
- [x] 摘要接口、loader 和 hook 收敛为 summary，接入 catalog 优先查询。
- [x] 选择器就近加载 list、读取 summary，移除自动默认赋值和父级列表依赖。
- [x] 参数、音色、视觉能力等消费者迁移到就近 detail。
- [x] 新建及功能开启入口显式获取并写入 default，防止异步覆盖用户输入。
- [x] 工作流 Context/props 不再下发模型数据，初始化、校验、导入导出和恢复在业务操作处读取。
- [x] 删除旧 hook、兼容参数及无引用工具，静态检查全仓消费者。
- [x] 局部测试覆盖冷启动、缓存命中/失败、并发、身份切换、默认优先级、失效引用和 UI 无隐式赋值；21 个测试文件、262 个用例通过。
- [x] App 类型检查、局部 ESLint（无 error，仍有 warning）、diff 空白检查通过；不主动运行全量测试。
- [ ] 浏览器页面验收：冷启动参数面板、TTS 音色、模型切换、新建知识库与工作流保存/调试。
