...
3. 再次压缩时始终使用运行时传入的最新 plan；checkpoint 中的标签只向模型提供上下文，不反序列化为运行时状态。
4. 完整 plan 只保存在独立 `value.plan` 中，作为 ChatItem 内唯一运行时状态源；`agentPlanUpdate` 仅用于还原工具调用历史，memory 不再保存 plan。
5. agent-loop 在 event/messages 转成最终 `assistantResponses` 时只保留最后一条 plan 快照；全部步骤为 `done/skipped` 时将其收敛为 `plan: null`，saveChat 只负责持久化结果。
6. 普通下一轮从未裁剪的 histories 倒序查找最后一个包含 plan 字段的 value：未完成 plan 仅用于初始化 agent-loop 的运行时 `activePlan`，不改写请求消息；完成 plan 或 null 都立即停止且不查更早状态。
7. 普通可见 user 文本和 checkpoint 中的 `<active_plan>` 不得反序列化为运行时状态。
8. `contextCheckpoint` history value 保存完整组合文本，历史恢复后仍只生成一条隐藏 `user` message。
...
- [x] 将运行时 active plan 确定性拼接到 checkpoint 前部
- [x] assistantResponses 输出层只保留最后一条独立 plan 状态，并从完整 histories 跨轮恢复未完成计划
- [x] 限制 checkpoint 和可见用户文本不得反序列化为运行时 plan 状态
