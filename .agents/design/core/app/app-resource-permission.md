...
本节只约束本设计中的 App 资源迁移，不改变注册表中其他已发布系统迁移的既有语义。该迁移以 `20260916_backfill_app_resource_snapshots` 作为永久任务 ID、首次发布版本为 4.17.1，并作为新的只回填任务追加到注册表末尾，不能修改已经发布任务。
...
- 历史 Version 与 App 表回填快照时，按对应发布人/创建者 `tmbId` 执行静默权限校验：操作人无读取权限的资源直接从 `resources` 快照中静默剔除（保持在 nodes 中），不报错、不中断迁移流水线；若无法查到有效成员信息（如成员已离职或不存在），按安全兜底规则将资源全部剔除为 `[]`，防止历史越权引用被自动合法化。实体缺失或停用仍由编辑器提示和运行时按需报错。
...
比较基准统一为 **该 App `time` 最新 Version**（含自动保存记录）。已有合法快照直接使用；缺 `resources` 时按第 4 节 extract 并执行与 6.1 节迁移一致的静默权限过滤（按 draft 责任人 `tmbId` 过滤）得到 baseline，防止历史未迁移草稿中的未授权引用被误判为 `kept` 从而绕过鉴权并固化到快照中。
