...
- 工作流图迁到 Version，App 用 `publishedVersionId` 和 `draftVersionId` 指向当前版；
- 保存/发布鉴权改为相对 `draftVersionId` 的增量校验（第 7 节）。
...
- `draftVersionId`：当前编辑器工作副本。自动保存、普通保存、保存并发布都更新为刚写入的那条 Version。没有指针时回退为该 App `time` 最新的 Version（含 `isAutoSave`）。

`draftVersionId` 就是比较和编辑器用的「最新版」。发布后尚未再保存时，它可以与 `publishedVersionId` 相同；下一次自动保存会指向 autoSave 那条，两者再次分开。

去掉：

- `modules / edges / chatConfig`（迁移并改读路径后 `$unset`）
- `resourceRefs`（本次重构前已存在，4161 `$unset`）

文件夹没有工作流，不写 Version，也没有这些图字段。

`pluginData.nodeVersion` 与 `draftVersionId` / `publishedVersionId` 的职责不得混用：正式运行只看 `publishedVersionId`（或等价的最新正式 Version 查询），不看 `nodeVersion`。
...
| 打开编辑器、复制工作流 | `draftVersionId`，否则该 App `time` 最新（含 autoSave） |
...
编辑器详情 `GET /core/app/detail`、创建 `POST /core/app/create`、画布和工具编辑都直接用 `nodes`，与 Version 同名。不要再把 Version.nodes 映射成 App.modules。`apps.modules` 只作为 4161 `$unset` 的历史字段。
...
| 操作 | Version | App |
| --- | --- | --- |
| 自动保存 | upsert `isAutoSave: true`，写入图 + 增量后的 `resources` | 更新 `draftVersionId`、`updateTime`；**不改** `publishedVersionId` |
| 普通保存 | insert 非正式 Version，图 + 增量后的 `resources` | 同上 |
| 保存并发布 | insert `isPublish: true`，图 + 完整 `resources`（新增无权限则整次失败） | 更新 `publishedVersionId`、`draftVersionId`、定时触发等 |
| 创建非文件夹 App | 首条 `isPublish: true` | 设好 `publishedVersionId` 和 `draftVersionId` |
| 原地类型转换 | 必须写 Version（upsert autoSave 或 insert 非正式），禁止只改 App | 不存图 |
| MCP/HTTP 更新 | `updateOne` **同一条** Version 的 `nodes` + `resources: []` | 不存图；无产品版本历史 |
...
| 存储 | 含义 | 行为 |
| --- | --- | --- |
| 字段不存在 / `null` / 非数组 | 未迁 | 与 4161 `buildResources` 相同：对该条 `nodes + chatConfig` 做 `extractAppResources`；若仍有 `resourceRefs.skillIds` 则 merge |
| `[]` | 已迁或已保存，明确无引用 | 空快照，不再从 nodes 提取 |
| 合法非空数组 | 已有快照 | `parse` 后直接用 |

不要抛 `App resources are not migrated`。
不要把缺字段当成 `[]`。

允许读时懒回写 `$set.resources`，避免每次 extract。4161 仍可批量回填并 `$unset resourceRefs`。
...
- 入口批量加载实体；缺失、软删、跨团队在入口失败。root Test/Debug 才允许跨团队，并向下传 `isRoot`。
...
- 指针回填 OCC：只在 `draftVersionId` / `publishedVersionId` 仍为空或仍是本次扫描结果时写入，避免覆盖并发保存/发布。
...
比较基准统一为 **`draftVersionId` 对应 Version**（即当前最新草稿）。没有指针时用该 App `time` 最新的 Version。缺 `resources` 时按第 4 节 extract 得到 baseline。

```text
extracted = extractAppResources(nodes, chatConfig)
baseline  = draftVersionId 的 resources
...
相对同一个 `draftVersionId` 做增量。`added` 里任一无权限，**整次发布失败**，指出缺权限的资源，不写正式 Version，不改 `publishedVersionId`。
...
2. **指针**：发布/创建写入 `publishedVersionId`；4161 回填。
3. **反查改查正式 Version**。
4. **零 Version 补建正式 Version**。
5. 编辑器/复制/Chat 回退改读 Version；类型转换/MCP 更新只写 Version。
6. `$unset` App 上的图字段和 `resourceRefs`。

第 1 步可先于清 App 上线，聊天不必等 4161 跑完。4–6 是结构重构，要按序，不能先清空再改读。

增量鉴权（第 7 节）与保存接口同一批改。`draftVersionId` 在创建/保存/自动保存/发布时写入，4161 回填为当时最新 Version。
...
1. 读时懒回写 `resources` 本次不做。4161 批量回填即可；运行时缺字段每次 extract。
...
- [x] App 增加并维护 `publishedVersionId`、`draftVersionId`（创建/保存/自动保存/发布/4161）。
