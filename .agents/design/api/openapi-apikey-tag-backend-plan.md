...
9. API Key list 继续接受旧版 `appId` 参数用于接口兼容，但不改变排序、可见范围，也不作为过滤条件。
...
8. API Key list 继续接受旧版 `appId` 参数用于接口兼容，不过滤其他 Key，也不影响排序。
...
- 不影响列表排序，排序只由 `sortBy` 决定。
- 如果同时传 `keyword`、`tags`，仍按这些条件筛选。
...
sortBy: z.enum(['createTime', 'lastUsedTime', 'remainingPoints']).default('createTime').meta({
  example: 'createTime',
  description: '排序字段；createTime、lastUsedTime 按倒序排序，remainingPoints 按升序排序'
})
```
...
- `appId` 仅兼容旧版调用，不过滤、不排序。
- `sortBy` 默认 `createTime`，可选 `lastUsedTime`、`remainingPoints`；时间越近越靠前，剩余积分越少越靠前。
...
1. `parseApiInput` 解析 `keyword`、`tags`、`sortBy`，并允许旧版 `appId` 参数通过 schema 校验。
...
8. 根据 `sortBy` 排序：创建时间、最后使用时间按倒序，剩余积分按升序。
9. `appId` 不参与排序，应用发布页和账号 API Key 页看到一致的排序结果。
...
1. 默认值：`按创建时间`。
2. 可选：`按最后使用时间`、`按剩余积分`。
3. 从应用发布页进入时不再传 `appId` 给 list；排序控件直接决定完整列表顺序。
...
应用发布页或应用详情页进入 API Key 表格时，不再传当前 `appId`，保证和账号 API Key 页排序一致：

```ts
getOpenApiKeys({ sortBy })
```
...
13. API Key list 传 `appId` 时，不影响排序，也不筛掉其他 Key。
...
29. API Key list 同时传 `appId` 和 `sortBy` 时，排序仍只由 `sortBy` 决定。
...
- [x] list Key 兼容旧版 `appId` 参数，但不让它影响排序或可见范围。
- [x] list Key 支持创建时间、最后使用时间、剩余积分排序：时间越近越靠前，剩余积分越少越靠前。
