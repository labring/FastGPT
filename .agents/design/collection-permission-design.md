...
2. **短路判定**（任选其一，降序）：
   - 团队所有者：全部可读（团队管理员无旁路，按其 collection 快照解析）；
...
2. **解析可读 collection 集合**：`resolveReadableCollectionIds`（auth.ts）输入 `teamId/datasetIds/tmbId` →（工作流检索仅在 `authTmbId` 开启、即存在真实成员身份时调用；未开启则不做 collection 级过滤，按 dataset 全量召回）
   - 团队 owner：返回 `undefined`（无 collection 级过滤，按 dataset 召回）；
...
- 短路：团队 owner / dataset 处于关闭态（`collectionPermissionEnabled !== true`，含全部存量 dataset）——O(1) 跳过 distinct 查询。
...
| 短路 | 关闭态（`collectionPermissionEnabled !== true`）列表/检索短路；团队 owner 短路（团队管理员不短路，与 dataset / app 层一致） |
