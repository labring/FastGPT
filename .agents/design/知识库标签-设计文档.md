...
- **MongoDB 粗筛**：根据条件类型区分正向肯定条件与负向/空值条件。
  - **肯定型操作符**（`$eq`, `$gt`, `$gte`, `$lt`, `$lte`, `$contains`, `$startsWith`, `$endsWith`, `$regex`, `$is`, `$in`, `$notEmpty`）：要求集合必须打上该标签且值匹配。
  - **否定/允许未打标型操作符**（`$empty`, `$ne`, `$isNot`, `$notContains`, `$notIn`）：未打上该标签的集合在业务语义上也算作满足。
  - 粗筛策略：
    - 若 `$and` 中包含至少一个肯定型条件，仅提取肯定型标签的 ID 进行 `{ 'tags.tagId': { $all: positiveAndTagIds } }` 查询并强制走 `.hint({ teamId: 1, datasetId: 1, 'tags.tagId': 1 })` 索引，快速将候选集压缩到极小范围；
    - 若查询全为否定/空值型条件（纯负向查询），或者包含否定条件的 `$or` 分支，则不施加 `tags.tagId` 粗筛约束，直接按 `{ teamId, datasetId }` 索引拉取候选集，避免未打标集合在数据库阶段被漏检。
- **应用层精筛**：
  - 若标签在当前知识库中未定义，条件视为不满足（返回 `false`），防止非法或已删除标签造成意外全集召回；
  - 若集合**未打上该标签**（`tags` 数组中无此 `tagId`）：
    - 否定/空值型操作符（`$empty`, `$ne`, `$isNot`, `$notContains`, `$notIn`）判定为**满足（true）**；
    - 肯定型操作符（`$eq`, `$is`, `$contains`, `$notEmpty`, `$gt` 等）判定为**不满足（false）**；
  - 若集合打上了该标签，则调用 `checkValue` 进行类型与值精确比对。对于打标但值为空/无法解析的情况，否定条件（如 `$ne`、`$isNot`、`$notContains`）同样视为满足，保持与未打标状态的行为自洽。
...
- [x] 运行局部测试、App typecheck、相关 lint 和 `git diff --check` 验证。
- [x] 否定型与空值型标签过滤支持未打标集合：调整 `collectionFilter.ts`，定义正负向操作符分类，支持未打标（absent）集合在 `$empty`、`$ne`、`$isNot`、`$notContains`、`$notIn` 下判定满足，在肯定型操作符下判定不满足。
- [x] MongoDB 预过滤分流优化：仅针对 `$and` 中的肯定型操作符提取 `tagId` 构造 `tags.tagId: { $all }` 复合索引粗筛；纯否定或包含否定操作符的分支放行不加 tagId 强制拦截，避免漏检未打标集合。
- [x] 补充与更新 `collectionFilter.test.ts` 单元测试，覆盖多选“不是”、数字“不等于”、时间“不是”、文本“不包含”、“为空”召回未打标集合，及“不为空”排除未打标集合等场景。
- [x] 运行局部测试与代码规范检查验证。
