...
4. 支持 `short-proxy`、`short-redirect`、`presigned` 三种下载模式。
...
| `STORAGE_EXTERNAL_ENDPOINT` / `STORAGE_S3_CDN_ENDPOINT` | redirect/presigned 使用的客户端可访问地址 |
...
| `packages/service/common/s3/buckets/base.ts` | 统一签发上传短链和三种下载 URL |
...
2. 默认使用 `short-proxy`；外部 S3/CDN 部署可切换 `short-redirect`；仅兼容旧消费者时使用 `presigned`。
