...
5. 如果内容无法证明类型，且策略没有安全 fallback，必须拒绝。

## 3. 目标

1. 无后缀文件不再在预签阶段被直接拒绝。
2. 无后缀文件上传时，如果内容可识别且策略允许，应上传成功。
3. 有后缀但内容不匹配时，仍然拒绝或按策略修正 metadata。
...
3. `size`：浏览器 `File.size`，用于校验和日志，不作为类型裁决核心。

### 7.3 UploadFileEvidence
...
export const ResolvedUploadFileSchema = z.object({
  ...
  extension: z.string(),
  detectionSource: z.enum(['magic', 'office-zip', 'text', 'hint', 'fallback']),
...
2. 解析 filename extension。
3. 如果 filename 有后缀且不在 allowedExtensions：
   - 可以继续在预签阶段拒绝。
   - 原因：这是明确违反业务策略的强 hint。
4. 如果 filename 没后缀：
   - 不拒绝。
   - 设置 `allowMissingExtension=true`。
5. 如果传了 `contentType` 且能映射到 allowedExtensions：
...
7. 如果 allowedExtensions 中包含文本类扩展名：
...
| `packages/global/openapi/core/chat/file/api.ts` | Chat 上传预签增加 `contentType/size` |
| `projects/app/src/pages/api/core/chat/file/presignChatFilePostUrl.ts` | 解析 hint 并传给 S3 source |
| `packages/service/common/s3/sources/chat/type.ts` | Chat upload schema 增加 contentType/size |
...
1. Chat OpenAPI 增加 `contentType/size`。
...
6. Dataset 下游读取 metadata 后 extension 正确。
7. ChatBox 和 FileSelector 的上传预签都传了 hint。
8. 前端错误提示保持稳定。
...
- [ ] PR2-T1 新增 `packages/service/common/s3/uploadPolicy/type.ts`。
- [ ] PR2-T2 新增 `packages/service/common/s3/uploadPolicy/utils.ts`，迁移 MIME/extension/text/OOXML 纯函数。
- [ ] PR2-T3 新增 `packages/service/common/s3/uploadPolicy/service.ts`，实现策略构建、evidence 检测和最终裁决。
- [ ] PR2-T4 调整 `UploadConstraintsSchema` 或新增 `UploadPolicySchema`，让 upload session 保存策略和 fileHint。
- [ ] PR2-T5 改造 `createUploadConstraints()`，缺后缀不再预签拒绝；有明确不允许后缀仍拒绝。
- [ ] PR2-T6 改造 `validateUploadFile()`，移除内容检测前的后缀 gate。
- [ ] PR2-T7 改造 `getUploadInspectBytes()`，基于 hint/policy 判断 OOXML 大窗口。
- [ ] PR2-T8 改造 `handleS3ProxyUpload()`，传入 fileHint/uploadPolicy 并写入最终 metadata。
- [ ] PR2-T9 Chat 上传预签 OpenAPI schema 增加 `contentType/size`。
- [ ] PR2-T10 Chat 预签 API 使用 `parseApiInput` 解析 hint 并传给 S3 source。
- [ ] PR2-T11 `S3ChatSource.createUploadChatFileURL()` 接收并传递 contentType/size hint。
- [ ] PR2-T12 ChatBox `useFileUpload` 预签时传 `rawFile.type/rawFile.size`。
- [ ] PR2-T13 FileSelector 预签时传 `rawFile.type/rawFile.size`。
- [ ] PR2-T14 补充 uploadPolicy/uploadValidation/uploadConstraints 单测。
- [ ] PR2-T15 补充 Chat 预签 API 单测。
- [ ] PR2-T16 补充短上传 proxy route 无后缀上传测试。
- [ ] PR2-T17 回归旧 JWT upload route 和错误解析。
- [ ] PR2-T18 运行 PR2 局部测试和 app typecheck。
...
这个设计能解决无后缀文件被过早拒绝的问题，同时保持上传安全边界：只要策略要求白名单，文件内容必须能证明自己属于白名单；证明不了就拒绝。
