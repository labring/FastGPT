...
1. 启动时默认执行 manager 管理的主动安全同步；保留 `SYNC_INDEX` 布尔变量用于有额外部署约束时整体关闭启动同步。
2. 移除 `MONGO_INDEX_SYNC_MODE`，不再通过环境变量切换索引处理模式；`SYNC_INDEX` 只控制是否执行安全同步。
...
4. 不再保留启动期模式选择或强制全量同步能力；只保留一个整体启停开关。
...
- 不提供环境变量切换到 Mongoose 全量同步或分别控制创建、清理阶段。

## 推荐方案

### 1. 可关闭的主动安全同步

移除 `MONGO_INDEX_SYNC_MODE`，主应用和 Marketplace 不再支持 `create/dryRun/sync` 等启动模式。两者统一保留 `SYNC_INDEX: BoolSchema.default(true)`：为 `true` 时执行同一条安全同步链路，为 `false` 时跳过该 model 的差异检查、索引创建和废弃清理。
...
在 Mongo 公共模块提供 `defineIndex` wrapper，统一表达当前索引与废弃索引。`deprecated` 默认是 `false`，省略时代理 `schema.index(key, options)`；只有显式设置 `deprecated: true` 时才保存清理元数据，不把该索引加入 Mongoose Schema。

所有由 FastGPT 管理的 Schema 索引都必须经过这个 wrapper，包括原先通过字段级 `index: true` 或 `unique: true` 隐式生成的索引。字段级索引需要改为相邻的显式 `defineIndex` 声明，确保索引创建、同步开关和废弃清理只有一个入口。
...
废弃索引名默认按 MongoDB 规则由 `key` 推导，也可以通过 `options.name` 显式指定。wrapper 只把 `key`、索引名以及需要参与精确匹配的关键 options 写入清理元数据。
...
- 移除 `MONGO_INDEX_SYNC_MODE`，保留默认开启的 `SYNC_INDEX` 作为安全同步总开关。
- 固定同步语义为 `createIndexes()` + 当前 Schema 显式声明的 deprecated cleanup。
- 使用 `defineIndex(schema, { key, options, deprecated })` 统一声明当前索引和废弃索引。
- 将主 Service、Marketplace 和 `pro/admin` 中由 FastGPT 管理的现有 Schema 索引迁移到 `defineIndex`，不保留直接 `schema.index()` 或字段级隐式索引声明。
...
1. 开启同步会创建 Schema 中缺失索引，并保留未登记的 schema 外索引。
...
8. 主应用与 Marketplace 均在 `SYNC_INDEX=true` 时调用同一 manager，关闭时跳过同步。
...
- [x] 确认修订方案：移除模式变量，保留安全同步总开关，废弃索引下沉到所属 Schema。
- [x] 新增 `defineIndex` wrapper 与 schema-level 废弃索引读取能力；定义仅包含 name、key 和用于精确匹配的可选关键 options。
- [x] 将主 Service、Marketplace 和 `pro/admin` 的当前索引与字段级隐式索引统一迁移为 `defineIndex(...)`，正常索引省略默认值为 `false` 的 `deprecated`。
...
- [x] 从主应用和 Marketplace env 中移除 `MONGO_INDEX_SYNC_MODE`，保留默认开启的 `SYNC_INDEX` 布尔开关。
...
- [x] 运行索引管理相关局部测试与类型检查；按本次要求不运行全量测试。
