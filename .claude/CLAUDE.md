...
- **国际化**: 完整支持中文、英文和日文
- **接口一致性**: 前端为请求参数添加的必填项、默认值等校验逻辑，必须与后端接口保持一致。不能仅在前端做特殊处理，否则暴露接口后，直接调用 API 的客户会因缺少这些参数或默认值而导致接口行为不一致。如果必须不一致需要用户代码调用时候维护的需要补充代码注释说明，否则在提交代码前提示用户处理，必须拒绝提交代码
